...
// 后端保存输入结构（与 backend/internal/handler/task_evaluation_handler.go 对齐）
...
export function evalRuleConfigToMethods(config: EvalRuleConfig): EvalRuleMethodInput[] {
  ...
  return config.evaluationMethods.map((mk) => {
    ...
    // 关键资源字段始终使用顶层编辑状态，避免 methodResourceConfigs 中的旧值覆盖当前选择
    if (mk === "paper") {
...
export function distributeEvalRuleWeights(methods: string[], existing: Record<string, number>): Record<string, number> {
  ...
  // 保留已有方式的权重，仅对新增/删除后的剩余权重重新分配（可选行为）
  return weights
}
