...
export const examApi = {
  ...createContentApi<
    Exam,
    Omit<Exam, 'id' | 'totalScore' | 'createdAt' | 'updatedAt'>,
    Partial<Omit<Exam, 'id' | 'totalScore' | 'createdAt' | 'updatedAt'>>
  >('/evaluation/exams'),
...
export const evaluationResultApi = {
  ...
  submit: (req: {
    ...
    request<SceneEvaluationResult>('/evaluation/results', {
      method: 'POST',
      body: JSON.stringify(req),
    }),
  grade: (
  ...
  batchGrade: (
    items: { id: string; score: number; evalPointScores?: Record<string, any>; comment?: string }[],
  ) =>
...
export const certApi = {
  listRules: (params?: {
    ...
    request<ListResponse<CertificationRule>>(
      `/evaluation/certifications${buildQuery(params || {})}`,
    ),
  ...
  updateRule: (id: string, req: { careerPositionId: string; ruleSource: string }) =>
    request<CertificationRule>(`/evaluation/certifications/${id}`, {
      method: 'PUT',
      body: JSON.stringify(req),
    }),
  ...
  listItems: (ruleId: string) =>
    request<ListResponse<CertificationAbilityItem>>(`/evaluation/certifications/${ruleId}/items`),
  upsertItem: (ruleId: string, req: Partial<CertificationAbilityItem>) =>
    request<CertificationAbilityItem>(`/evaluation/certifications/${ruleId}/items`, {
      method: 'POST',
      body: JSON.stringify(req),
    }),
  updateItem: (id: string, req: { name: string; sortOrder: number }) =>
    request<CertificationAbilityItem>(`/evaluation/certifications/items/${id}`, {
      method: 'PUT',
      body: JSON.stringify(req),
    }),
  deleteItem: (id: string) =>
    request<{ id: string }>(`/evaluation/certifications/items/${id}`, { method: 'DELETE' }),
  listPoints: (itemId: string) =>
    request<ListResponse<CertificationAbilityPoint>>(
      `/evaluation/certifications/items/${itemId}/points`,
    ),
  updatePoint: (id: string, req: CertificationPointPayload) =>
    request<CertificationAbilityPoint>(`/evaluation/certifications/points/${id}`, {
      method: 'PUT',
      body: JSON.stringify(req),
    }),
  deletePoint: (id: string) =>
    request<{ id: string }>(`/evaluation/certifications/points/${id}`, { method: 'DELETE' }),
  createTask: (pointId: string, req: CertificationTaskPayload) =>
    request<CertificationRelatedTask>(`/evaluation/certifications/points/${pointId}/tasks`, {
      method: 'POST',
      body: JSON.stringify(req),
    }),
  updateTask: (id: string, req: CertificationTaskPayload) =>
    request<CertificationRelatedTask>(`/evaluation/certifications/tasks/${id}`, {
      method: 'PUT',
      body: JSON.stringify(req),
    }),
  deleteTask: (id: string) =>
    request<{ id: string }>(`/evaluation/certifications/tasks/${id}`, { method: 'DELETE' }),
  getFullRule: (id: string) =>
    request<CertificationFullRuleResponse>(`/evaluation/certifications/${id}/full`),
  putFullRule: (id: string, req: CertificationFullRulePayload) =>
    request<CertificationRule>(`/evaluation/certifications/${id}/full`, {
      method: 'PUT',
      body: JSON.stringify(req),
    }),
...
export const jobAbilityResultApi = {
  list: (params?: {
    ...
    request<ListResponse<JobAbilityResult>>(
      `/evaluation/job-ability/results${buildQuery(params || {})}`,
    ),
...
export const portraitApi = {
  list: (params?: {
    ...
    request<ListResponse<StudentAbilityPortrait>>(
      `/evaluation/portraits${buildQuery(params || {})}`,
    ),
  get: (id: string) => request<StudentAbilityPortrait>(`/evaluation/portraits/${id}`),
  generate: (userId: string, careerPositionId: string) =>
    request<StudentAbilityPortrait>('/evaluation/portraits/generate', {
      method: 'POST',
      body: JSON.stringify({ userId, careerPositionId }),
    }),
  listArchives: (params?: {
    userId?: string
    materialType?: string
    ...
    request<ListResponse<StudentAbilityArchive>>(
      `/evaluation/portraits/archives${buildQuery(params || {})}`,
    ),
  upsertArchive: (req: Partial<StudentAbilityArchive>) =>
    request<StudentAbilityArchive>('/evaluation/portraits/archives', {
      method: 'POST',
      body: JSON.stringify(req),
    }),
...
export const evaluationBatchApi = {
  ...createCrudApi<
    EvaluationBatch,
    Omit<EvaluationBatch, 'id' | 'createdAt' | 'updatedAt'>,
    Partial<Omit<EvaluationBatch, 'id' | 'createdAt' | 'updatedAt'>>
  >('/evaluation/batches'),
  updateStatus: (id: string, status: string) =>
    request<EvaluationBatch>(`/evaluation/batches/${id}/status`, {
      method: 'POST',
      body: JSON.stringify({ status }),
    }),
}
