...
export const portalUserManagementApi = {
  ...
  create: (req: CreateUserRequest) =>
    portalRequest<User>('/users', { method: 'POST', body: JSON.stringify(req) }),
  update: (id: string, req: Partial<CreateUserRequest>) =>
    portalRequest<User>(`/users/${id}`, { method: 'PUT', body: JSON.stringify(req) }),
  delete: (id: string) => portalRequest<{ id: string }>(`/users/${id}`, { method: 'DELETE' }),
  updateStatus: (id: string, status: string) =>
    portalRequest<User>(`/users/${id}/status`, {
      method: 'POST',
      body: JSON.stringify({ status }),
    }),
  resetPassword: (id: string, password: string) =>
    portalRequest<User>(`/users/${id}/reset-password`, {
      method: 'POST',
      body: JSON.stringify({ password }),
    }),
  bindRoles: (id: string, roleIds: string[]) =>
    portalRequest<User>(`/users/${id}/roles`, {
      method: 'POST',
  ...
  batchGraduate: (req: { userIds: string[]; graduateYear?: number }) =>
    portalRequest<{ count: number }>('/users/batch-graduate', {
      method: 'POST',
      body: JSON.stringify(req),
    }),
  batchDelete: (userIds: string[]) =>
    portalRequest<{ count: number }>('/users/batch-delete', {
      method: 'POST',
      body: JSON.stringify({ userIds }),
    }),
  batchUpdateOrgNode: (req: { userIds: string[]; orgNodeId?: string }) =>
    portalRequest<{ count: number }>('/users/batch-org-node', {
      method: 'POST',
      body: JSON.stringify(req),
    }),
}
...
export const portalStaffTitleApi = {
  ...
  toggleStatus: (id: string, status: string) =>
    portalRequest<StaffTitle>(`/staff-titles/${id}/status`, {
      method: 'POST',
      body: JSON.stringify({ status }),
    }),
}
...
export const portalUserExtensionFieldApi = {
  list: (params?: { tenantId?: string }) =>
    portalRequest<ListResponse<UserExtensionField>>(
      `/user-extension-fields${buildQuery(params || {})}`,
    ),
  update: (
    id: string,
    req: Partial<
      Omit<
        UserExtensionField,
        'id' | 'tenantId' | 'slotNumber' | 'fieldKey' | 'fieldType' | 'createdAt'
      >
    >,
  ) =>
    portalRequest<UserExtensionField>(`/user-extension-fields/${id}`, {
      method: 'PUT',
      body: JSON.stringify(req),
    }),
}
...
export const portalUserRelationApi = {
  ...
  delete: (id: string) =>
    portalRequest<{ id: string }>(`/user-relations/${id}`, { method: 'DELETE' }),
}
