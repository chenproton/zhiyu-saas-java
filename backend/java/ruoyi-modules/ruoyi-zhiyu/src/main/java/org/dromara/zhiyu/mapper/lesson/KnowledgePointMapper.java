...
public interface KnowledgePointMapper extends BaseMapperPlus<KnowledgePoint, KnowledgePoint> {
    ...
    /** 同步颗粒课对知识点的双向引用：移除。 */
    @Update("UPDATE courses SET knowledge_point_ids = JSON_REMOVE(knowledge_point_ids, JSON_UNQUOTE(JSON_SEARCH(knowledge_point_ids, 'one', #{kpId}))), updated_at = NOW()"
        + " WHERE tenant_id = #{tenantId} AND id <> ALL(#{courseIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler})"
        + " AND #{kpId} = ANY(knowledge_point_ids)")
    ...
    @Update("UPDATE knowledge_points SET granular_lesson_ids = JSON_REMOVE(granular_lesson_ids, JSON_UNQUOTE(JSON_SEARCH(granular_lesson_ids, 'one', #{courseId}))), updated_at = NOW()"
        + " WHERE tenant_id = #{tenantId} AND id <> ALL(#{kpIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler})"
        + " AND #{courseId} = ANY(granular_lesson_ids)")
