...
/**
 * 场景批次服务实现（对齐 Go batch_handler.go + NewSceneBatchTableConfig +
 * store/batches.go 语义）。
 *
...
public class SceneBatchServiceImpl implements ISceneBatchService {
    ...
    public ListResponse<BatchDto> list(String orgNodeId, String status, String search, long limit, long offset) {
        ...
        List<SceneBatch> rows = batchMapper.selectBatchPage(tenantId, orgNodeId, status, pattern,
            (int) safeLimit, (int) safeOffset);
        List<BatchDto> items = new ArrayList<>(rows.size());
        for (SceneBatch b : rows) {
            items.add(toDto(b));
        }
    ...
    public BatchDto create(BatchCreateRequest req) {
        ...
        SceneBatch batch = new SceneBatch();
        ...
        // 场景批次创建恒为 open（对齐 Go CreateWithStatus=false）
        batch.setStatus("open");
        batch.setTenantId(tenantId);
        batchMapper.insert(batch);
        SceneBatch created = batchMapper.selectItemById(batch.getId());
        if (created == null) {
    ...
    private SceneBatch fetchOwned(String id) {
        SceneBatch batch = batchMapper.selectItemById(id);
        if (batch == null) {
    ...
    private BatchDto toDto(SceneBatch b) {
        ...
        dto.setScenarioCount(b.getScenarioCount());
