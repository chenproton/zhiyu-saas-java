...
public interface LessonBehaviorRecordMapper extends BaseMapperPlus<LessonBehaviorRecord, LessonBehaviorRecord> {
    ...
    @Select("<script>SELECT r.id, r.course_id, r.student_user_id, u.name AS student_name, r.record_date,"
        ...
        + " WHERE r.course_id = #{courseId}::uuid AND r.tenant_id = #{tenantId}::uuid"
    ...
    @Insert("INSERT INTO lesson_behavior_records"
        ...
        + " VALUES (gen_random_uuid(), #{tenantId}::uuid, #{courseId}::uuid, #{studentUserId}::uuid, #{recordDate},"
        ...
        + " DO UPDATE SET attendance = EXCLUDED.attendance, quiz_score = EXCLUDED.quiz_score,"
        + " interaction_count = EXCLUDED.interaction_count, praise_count = EXCLUDED.praise_count,"
        + " rush_correct_count = EXCLUDED.rush_correct_count, rush_avg_time_sec = EXCLUDED.rush_avg_time_sec,"
        + " updated_at = NOW()")
    ...
    @Select("SELECT id, course_id, student_user_id, record_date, attendance, quiz_score, interaction_count,"
        ...
        + " FROM lesson_behavior_records WHERE course_id = #{courseId}::uuid AND student_user_id = #{studentUserId}::uuid"
