...
public interface AffairsScheduleImportMapper {
    ...
    @Select("SELECT EXISTS(SELECT 1 FROM terms WHERE id = #{termId} AND tenant_id = #{tenantId})")
    boolean termExists(@Param("tenantId") String tenantId, @Param("termId") String termId);

    @Select("SELECT p.term_id FROM teaching_plan_entries e JOIN teaching_plans p ON p.id = e.plan_id"
        + " WHERE p.tenant_id = #{tenantId} AND e.course_name = #{courseName} LIMIT 1")
    String inferTermByCourseName(@Param("tenantId") String tenantId, @Param("courseName") String courseName);

    @Select("SELECT e.id AS id, e.teacher_id AS teacher_id, e.class_node_id AS class_node_id,"
        + " e.scenario_id AS scenario_id, e.course_id AS course_id, e.course_code"
        + " FROM teaching_plan_entries e JOIN teaching_plans p ON p.id = e.plan_id"
        + " WHERE p.tenant_id = #{tenantId} AND p.term_id = #{termId} AND e.course_name = #{courseName} LIMIT 1")
    ...
    @Select("SELECT id FROM users WHERE tenant_id = #{tenantId} AND (name = #{name} OR username = #{name}) LIMIT 1")
    ...
    @Insert("INSERT INTO schedule_entries (id, tenant_id, term_id, plan_entry_id, course_name, course_code, course_id, type,"
        ...
        + " VALUES (#{id}, #{tenantId}, #{termId}, #{planEntryId}, #{courseName}, #{courseCode}, #{courseId}, #{entryType},"
        + " #{firstClassId}, #{classNodeIds}, #{teacherId}, #{day}, #{periods}, #{startWeek}, #{endWeek}, #{weekPattern},"
    ...
    @Update("UPDATE teaching_plan_entries SET status = 'scheduled' WHERE id = #{planEntryId}")
    int markPlanEntryScheduled(@Param("planEntryId") String planEntryId);

    @Delete("DELETE FROM schedule_entries WHERE tenant_id = #{tenantId} AND term_id = #{termId}"
        + " AND status = 'draft'")
