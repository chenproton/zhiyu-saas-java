...
public interface EvaluationPortraitMapper extends BaseMapperPlus<EvaluationStudentPortrait, EvaluationStudentPortrait> {
    ...
    @Select("SELECT EXISTS(SELECT 1 FROM users WHERE id = #{userId}::uuid AND tenant_id = #{tenantId}::uuid)")
    boolean userInTenant(@Param("userId") String userId, @Param("tenantId") String tenantId);

    /** 批量学生班级/专业信息（对齐 Go ListProfiles，汇聚用） */
    @Select("<script>SELECT u.id::text AS user_id, COALESCE(o.name, '') AS class_name,"
        + " u.major_id::text AS major_id, COALESCE(m.name, '') AS major_name"
    ...
    @Select("SELECT r.career_position_id::text AS position_id, COALESCE(cp.name, '') AS position_name,"
        ...
        + " WHERE r.user_id = #{userId}::uuid ORDER BY r.achievement_rate DESC LIMIT 3")
    ...
    @Update("INSERT INTO student_ability_portraits (tenant_id, user_id, career_position_id, overall_grade,"
        ...
        + " tenant_id = EXCLUDED.tenant_id, overall_grade = EXCLUDED.overall_grade,"
        + " domain_scores = EXCLUDED.domain_scores, recommend_positions = EXCLUDED.recommend_positions,"
