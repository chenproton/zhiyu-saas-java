...
public interface AlliancePublicFileMapper {
    ...
    @Select("SELECT EXISTS ("
        ...
        + " WHERE tenant_id = #{tenantId}::uuid AND enable_public = true"
        ...
        + "        OR #{url} IN (SELECT jsonb_array_elements_text(x.photos)))"
        + " UNION ALL"
        + " SELECT 1 FROM alliance_achievements"
        + " WHERE tenant_id = #{tenantId}::uuid AND is_public = true"
        + "   AND (#{url} = cover_image OR #{url} IN (SELECT jsonb_array_elements_text(images)))"
        + " UNION ALL"
        + " SELECT 1 FROM alliance_projects"
        + " WHERE tenant_id = #{tenantId}::uuid AND is_public = true AND cover_image = #{url}"
        + " UNION ALL"
