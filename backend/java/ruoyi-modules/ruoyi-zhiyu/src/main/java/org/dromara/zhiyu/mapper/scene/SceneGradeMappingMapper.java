...
public interface SceneGradeMappingMapper extends BaseMapperPlus<SceneGradeMapping, SceneGradeMapping> {
    ...
    @Select("<script>SELECT id, scenario_id, task_id, level, min_score, max_score, description, color, tenant_id"
        + " FROM scenario_grade_mappings WHERE tenant_id = #{tenantId}::uuid"
        + " <if test=\"scenarioId != null and scenarioId != ''\">AND scenario_id = #{scenarioId}::uuid</if>"
        + " <if test=\"taskId != null and taskId != ''\">AND task_id = #{taskId}::uuid</if>"
    ...
    /** 查询映射所属场景 ID（归属校验用）。 */
    @Select("SELECT scenario_id FROM scenario_grade_mappings WHERE id = #{id}::uuid")
    ...
    @Insert("INSERT INTO scenario_grade_mappings (id, tenant_id, scenario_id, task_id, level, min_score, max_score,"
        + " description, color)"
        + " VALUES (gen_random_uuid(), #{tenantId}::uuid, #{scenarioId}::uuid, #{taskId}::uuid, #{level},"
    ...
    /** 更新等级映射（按 id）。 */
    @Update("UPDATE scenario_grade_mappings SET scenario_id = #{scenarioId}::uuid, task_id = #{taskId}::uuid,"
        + " level = #{level}, min_score = #{minScore}, max_score = #{maxScore}, description = #{description},"
        + " color = #{color} WHERE id = #{id}::uuid")
    ...
    /** 删除等级映射（限定租户，纵深防御）。 */
    @Delete("DELETE FROM scenario_grade_mappings WHERE id = #{id}::uuid AND tenant_id = #{tenantId}::uuid")
