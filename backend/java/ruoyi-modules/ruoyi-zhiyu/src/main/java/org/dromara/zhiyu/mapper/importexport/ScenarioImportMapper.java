...
public interface ScenarioImportMapper {
    ...
    @Select("SELECT id::text AS id, COALESCE(creator_id::text, '') AS creator_id, co_builder_ids::text AS co_builder_ids"
        + " FROM scenarios WHERE tenant_id = #{tenantId}::uuid AND name = #{name} LIMIT 1")
    Map<String, Object> selectScenarioIdentity(@Param("tenantId") String tenantId, @Param("name") String name);

    /** 按租户+名称查询场景 ID（rename 模式判重）。 */
    @Select("SELECT id::text FROM scenarios WHERE tenant_id = #{tenantId}::uuid AND name = #{name} LIMIT 1")
    String selectScenarioIdByName(@Param("tenantId") String tenantId, @Param("name") String name);

    /** 覆盖导入：更新场景基础字段（限定租户）。 */
    @Update("UPDATE scenarios SET name = #{name}, career_position_id = #{careerPositionId}::uuid,"
        ...
        + " batch_id = #{batchId}::uuid, difficulty = #{difficulty}, background = #{background}, updated_at = NOW()"
    ...
    @Delete("DELETE FROM task_evaluation_methods WHERE task_id IN"
        + " (SELECT id FROM scenario_tasks WHERE scenario_id = #{scenarioId}::uuid)")
    int deleteTaskEvalMethodsByScenario(@Param("scenarioId") String scenarioId);

    /** 覆盖导入：清空场景原有任务。 */
    @Delete("DELETE FROM scenario_tasks WHERE scenario_id = #{scenarioId}::uuid")
    ...
    @Insert("INSERT INTO scenarios (id, tenant_id, name, code, career_position_id, industry_ids, profession_ids,"
        ...
        + " VALUES (#{id}::uuid, #{tenantId}::uuid, #{name}, #{code}, #{careerPositionId}::uuid,"
        ...
        + " #{batchId}::uuid, #{difficulty}, 'V1.0', 'draft', #{background}, #{creatorId}::uuid, '{}'::uuid[])")
    ...
    @Insert("INSERT INTO scenario_tasks (id, tenant_id, scenario_id, name, code, sort_order, background,"
        ...
        + " VALUES (#{id}::uuid, #{tenantId}::uuid, #{scenarioId}::uuid, #{name}, #{code}, #{sortOrder},"
    ...
    @Insert("INSERT INTO task_evaluation_methods (id, tenant_id, task_id, method_key, weight, eval_object,"
        ...
        + " VALUES (#{id}::uuid, #{tenantId}::uuid, #{taskId}::uuid, #{methodKey}, #{weight}, 'individual', NULL,"
        + " '[]'::jsonb, NULL, '{}'::jsonb, 1, true)"
        ...
        + " eval_subjects = EXCLUDED.eval_subjects, rubric_template_id = EXCLUDED.rubric_template_id,"
        + " resource_config = EXCLUDED.resource_config, version = EXCLUDED.version,"
    ...
    @Select("SELECT id::text FROM majors WHERE tenant_id = #{tenantId}::uuid"
    ...
    @Select("SELECT id::text FROM scene_batches WHERE tenant_id = #{tenantId}::uuid AND name = #{name} LIMIT 1")
