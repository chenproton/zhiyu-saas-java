...
public interface EvaluationQuestionBankMapper extends BaseMapperPlus<EvaluationQuestionBank, EvaluationQuestionBank> {
    ...
    @Insert("INSERT INTO question_banks (id, tenant_id, code, name, description, cover_image, status, question_count,"
        ...
        + " CAST(#{collaboratorIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]),"
        + " CAST(#{collaboratorDeptIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]),"
    ...
    @Update("UPDATE question_banks SET name = #{name}, description = #{description}, cover_image = #{coverImage},"
        + " collaborator_ids = CAST(#{collaboratorIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]),"
        + " collaborator_dept_ids = CAST(#{collaboratorDeptIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]),"
    ...
    @Select("SELECT EXISTS(SELECT 1 FROM question_banks WHERE tenant_id = #{tenantId}::uuid AND code = #{code})")
    ...
    @Select("<script>SELECT bank_id, COUNT(*) AS cnt FROM questions WHERE bank_id IN"
        + " <foreach collection='ids' item='id' open='(' separator=',' close=')'>#{id}::uuid</foreach>"
    ...
    @Insert("<script>INSERT INTO question_bank_knowledge_points (id, question_bank_id, knowledge_point_id) VALUES"
        ...
        + " (gen_random_uuid(), #{bankId}::uuid, #{kp}::uuid)</foreach>"
        + " ON CONFLICT (question_bank_id, knowledge_point_id) DO NOTHING</script>")
    int insertKps(@Param("bankId") String bankId, @Param("kps") List<String> kps);

    @org.apache.ibatis.annotations.Delete("DELETE FROM question_bank_knowledge_points WHERE question_bank_id = #{bankId}::uuid")
    ...
    /** 邀请协作者（数组去重追加） */
    @Update("UPDATE question_banks SET collaborator_ids = array_append(collaborator_ids, #{userId}::uuid), updated_at = NOW()"
        + " WHERE id = #{id} AND NOT (collaborator_ids @> ARRAY[#{userId}::uuid])")
    ...
    @Select("SELECT COUNT(*) FROM question_banks WHERE tenant_id = #{tenantId}::uuid AND creator_id = #{creatorId}::uuid AND is_draft_pool = true")
