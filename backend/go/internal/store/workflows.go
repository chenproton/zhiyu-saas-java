...
// scanWorkflow 单行审批流程扫描（Get/ScanWorkflowRows 共用）。
...
// Create 创建审批流程（强制租户：审批流程必须绑定学校，不再支持全局流程）。
...
func (s *WorkflowStore) Delete(ctx context.Context, id, tenantID string) error {
	_, err := s.q.Exec(ctx, `DELETE FROM workflows WHERE id = $1 AND tenant_id = $2`, id, tenantID)
	return err
}
