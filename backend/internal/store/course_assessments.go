	...
	for i, qq := range questions {
		...
		if existingID != "" {
			if _, err := q.Exec(ctx, `
				...
				WHERE id = $8
			`, qq.qType, qq.content, string(qq.options), string(qq.answer), qq.analysis, qq.score, i+1, existingID); err != nil {
			...
			`, uuid.NewString(), tenantID, examID, qq.id, qq.qType, qq.content, string(qq.options), string(qq.answer), qq.analysis, qq.score, i+1); err != nil {
