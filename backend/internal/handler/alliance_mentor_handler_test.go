...
// TestGraduation_EnterpriseMentorBinding B14：毕业设计企业导师必须 ∈ 本校已启用 mentor_links 影子账号。
func TestGraduation_EnterpriseMentorBinding(t *testing.T) {
	env := testhelper.SetupTestEnv(t)
	defer env.Cleanup()
	ctx := context.Background()
	tenantID := testhelper.TestTenantID
	...
	if _, err := env.DB.Exec(ctx, `
		INSERT INTO users (id, tenant_id, role, platform, username, login_name, password_hash, name, status, title_ids)
		VALUES ($1,$2,'school','portal',$3,$4,'x','毕设测试导师','active','{}')
	`, mentorUserID, tenantID, "em_grad_"+mentorUserID[:8], tenantID+"_em_grad_"+mentorUserID[:8]); err != nil {
		t.Fatalf("预置影子账号失败: %v", err)
	}
	if _, err := env.DB.Exec(ctx, `
		INSERT INTO alliance_expert_mentor_links (id, tenant_id, expert_id, user_id, enabled)
	...
	if _, err := env.DB.Exec(ctx, `
		INSERT INTO career_positions (id, tenant_id, name, position_type, version, status, created_by, code)
		VALUES ($1,$2,'毕设测试岗位','profession','v1','published',$3,'GRAD-POS-01')
	`, posID, tenantID, testhelper.TestOperatorID); err != nil {
		t.Fatalf("预置岗位失败: %v", err)
	}
	defer env.DB.Exec(ctx, `DELETE FROM career_positions WHERE id = $1`, posID)

	st := store.New(env.DB)
	h := &handler.GraduationHandler{Service: service.NewEvaluationService(service.New(st))}
	r := chi.NewRouter()
	r.Post("/evaluation/graduation/topics", h.CreateTopic)
	r.Put("/evaluation/graduation/topics/{id}", h.UpdateTopic)

	claims := claimsWithRoles("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaa11", domain.RoleTeacher)

	t.Run("create with unbound mentor rejected", func(t *testing.T) {
		w := doWithClaims(r, http.MethodPost, "/evaluation/graduation/topics", map[string]interface{}{
			"name":               "毕设绑定校验课题",
			"careerPositionId":   posID,
			"source":             "scene",
			"capacity":           5,
			"enterpriseMentorId": uuid.NewString(),
		}, claims)
	...
	t.Run("create with enabled mentor ok", func(t *testing.T) {
		w := doWithClaims(r, http.MethodPost, "/evaluation/graduation/topics", map[string]interface{}{
			"name":               "毕设绑定校验课题",
			"careerPositionId":   posID,
			"source":             "scene",
			"capacity":           5,
			"enterpriseMentorId": mentorUserID,
		}, claims)
		if w.Code != http.StatusCreated {
			t.Fatalf("expected 201, got %d: %s", w.Code, w.Body.String())
		}
		var topic domain.GraduationProjectTopic
		...
		// 更新为非法导师 → 400
		w = doWithClaims(r, http.MethodPut, "/evaluation/graduation/topics/"+topic.ID, map[string]interface{}{
			"name":               "毕设绑定校验课题",
			"careerPositionId":   posID,
			"source":             "scene",
			"capacity":           5,
		...
		if w.Code != http.StatusBadRequest {
			t.Fatalf("update: expected 400, got %d: %s", w.Code, w.Body.String())
		}
	})
}
