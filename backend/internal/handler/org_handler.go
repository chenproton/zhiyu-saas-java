...
func (h *OrgHandler) Tree(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondError(w, http.StatusInternalServerError, "统计organization members失败")
		return
	}
	for i := range items {
...
func (h *OrgHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if req.ParentID != nil && *req.ParentID != "" {
		if *req.ParentID == id {
			respondError(w, http.StatusBadRequest, "不能将自身设为父节点")
			return
		}
		...
		if descendantOfSelf {
			respondError(w, http.StatusBadRequest, "不能将子节点设为父节点")
			return
		}
	}
...
func (h *OrgHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	`, id, org.TenantID)
	if err != nil {
		respondError(w, http.StatusInternalServerError, "收集子树组织失败")
		return
	}
	for rows.Next() {
		var subID string
		if err := rows.Scan(&subID); err != nil {
			respondError(w, http.StatusInternalServerError, "读取子树组织失败")
			return
		}
	...
	for _, sid := range subtreeIDs {
		...
		if err != nil {
			respondError(w, http.StatusInternalServerError, "无效组织ID")
			return
		}
