...
func NewHandlers(db *pgxpool.Pool, jwtSecret string, fileHandler *handler.FileHandler, redisClient *redis.Client) *Handlers {
	...
	return &Handlers{
		...
		orgTypeHandler:                &handler.OrgTypeHandler{Service: service.NewOrgTypeService(svc), Store: store.NewOrgTypesStore(db)},
		userManagementHandler:         &handler.UserManagementHandler{Service: service.NewUserService(svc)},
		roleHandler:                   &handler.RoleHandler{Service: service.NewRoleService(svc), Store: store.NewRolesStore(db)},
		majorHandler:                  &handler.MajorHandler{Service: service.NewMajorService(svc), Store: store.NewMajorsStore(db)},
		industryHandler:               &handler.IndustryHandler{Service: service.NewIndustryService(svc), Store: store.NewIndustriesStore(db)},
		...
		staffTitleHandler:             &handler.StaffTitleHandler{Service: service.NewStaffTitleService(svc), Store: store.NewStaffTitlesStore(db)},
		...
		certificateLibraryHandler:     &handler.CertificateLibraryHandler{Service: service.NewCertificateLibraryService(svc), Store: store.NewCertificateLibraryStore(db)},
		...
		learnRoadHandler:              &handler.LearnRoadHandler{Service: service.NewLearnRoadService(svc), Store: store.NewLearnRoadsStore(db)},
		...
		microCertHandler:              &handler.MicroCertHandler{Service: service.NewMicroCertService(svc), Store: store.NewMicroCertStore(db)},
		...
		onSiteQuestionLibraryHandler:  &handler.OnSiteQuestionLibraryHandler{Service: service.NewOnSiteQuestionLibraryService(svc), Store: store.NewOnSiteQuestionLibraryStore(db)},
