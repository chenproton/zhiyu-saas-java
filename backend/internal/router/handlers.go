...
func NewHandlers(db *pgxpool.Pool, jwtSecret string, fileHandler *handler.FileHandler) *Handlers {
	return &Handlers{
		...
		abilityHandler:                &handler.AbilityHandler{DB: db, Store: store.NewAbilitiesStore(db)},
		...
		abilityDomainHandler:          &handler.AbilityDomainHandler{DB: db, Store: store.NewAbilityDomainsStore(db)},
