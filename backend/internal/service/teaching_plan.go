...
func (s *AffairsPlanService) ListTeachingPlans(ctx context.Context, p store.ListParams, cfg store.ListQueryConfig[domain.TeachingPlan]) ([]domain.TeachingPlan, int, error) {
	return s.st.TeachingPlans().List(ctx, p, cfg)
}

// GetTeachingPlan 查询单个教学计划。
func (s *AffairsPlanService) GetTeachingPlan(ctx context.Context, id, tenantID string) (*domain.TeachingPlan, error) {
	return s.st.TeachingPlans().Get(ctx, id, tenantID)
}

// ListTeachingPlanEntries 查询计划条目。
func (s *AffairsPlanService) ListTeachingPlanEntries(ctx context.Context, planID, tenantID string) ([]domain.TeachingPlanEntry, error) {
	return s.st.TeachingPlans().ListPlanEntries(ctx, planID, tenantID)
}

// GetTeachingPlanEntry 查询单个计划条目。
func (s *AffairsPlanService) GetTeachingPlanEntry(ctx context.Context, id, tenantID string) (*domain.TeachingPlanEntry, error) {
	return s.st.TeachingPlans().GetPlanEntry(ctx, id, tenantID)
}

// UpdateTeachingPlanEntry 更新计划条目。
func (s *AffairsPlanService) UpdateTeachingPlanEntry(ctx context.Context, id, tenantID string, e *domain.TeachingPlanEntry, credits *float64, totalHours *int, classNodeIDs *[]string) error {
...
func (s *AffairsPlanService) DeleteTeachingPlanEntry(ctx context.Context, id, tenantID string) error {
	return s.st.TeachingPlans().DeletePlanEntry(ctx, id, tenantID)
}

// ConfirmTeachingPlan 确认计划。
func (s *AffairsPlanService) ConfirmTeachingPlan(ctx context.Context, id, tenantID string) error {
...
func (s *AffairsPlanService) FetchTeachingPlanProgramBrief(ctx context.Context, id, tenantID string) (*store.ProgramBrief, error) {
...
func (s *AffairsPlanService) FetchPositionScenarios(ctx context.Context, positionID string) ([]store.ScenarioBrief, error) {
	return s.st.TeachingPlans().FetchPositionScenarios(ctx, positionID)
}

// FindTeachingPlanExisting 查询已有计划。
func (s *AffairsPlanService) FindTeachingPlanExisting(ctx context.Context, programID, termID, tenantID string) (string, error) {
	return s.st.TeachingPlans().FindExistingPlan(ctx, programID, termID, tenantID)
}

// TeachingPlanScheduledCount 查询已排课条目数。
func (s *AffairsPlanService) TeachingPlanScheduledCount(ctx context.Context, planID string) (int, error) {
