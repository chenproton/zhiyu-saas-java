...
export function typeMetaFor(typeName?: string): { icon: React.ElementType; color: string; badge?: string } {
  ...
  return (typeName && map[typeName]) || { icon: Building, color: "text-slate-600", badge: "bg-slate-50 text-slate-700 border-slate-200" }
}
