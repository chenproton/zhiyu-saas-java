...
export default function PartnerCoBuildPositionsPage() {
  ...
  const doTransition = async (item: CoBuildPosition, action: 'submit' | 'withdraw') => {
    setActingId(item.id)
    try {
      if (action === 'submit') {
        await partnerCobuildPositionApi.submit(item.id)
        toast({ title: t('已提交审核') })
      } else {
        await partnerCobuildPositionApi.withdraw(item.id)
        toast({ title: t('已撤回') })
      }
      await refresh()
    } catch (e) {
  ...
  return (
    <PortalCrudPage
      title={t('岗位共建')}
      description={t('为已确认合作的学校创建岗位资源，提交后由学校审批生效。')}
      entityLabel={t('岗位')}
          ...
          <TableRowActions>
            ...
            {p.status === 'pending' && (
              <Button
                ...
                size="sm"
                disabled={actingId === p.id}
                onClick={() => doTransition(p, 'withdraw')}
              >
