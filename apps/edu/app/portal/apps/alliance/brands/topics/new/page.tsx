...
export default function AllianceBrandTopicNewPage() {
  const { tenantId } = usePortalAuth()
  const { toast } = useToast()
  const router = useRouter()
  const [saving, setSaving] = useState(false)
  const [brands, setBrands] = useState<AllianceBrand[]>([])
  const [item, setItem] = useState<AllianceBrandTopic>({
    id: "", tenantId: tenantId || "", name: "", theme: "", description: "",
    layout: "grid", coverImage: "", contentBlocks: [], relatedBrandIds: [],
    status: "draft", isRecommended: false, sortOrder: 0, createdAt: "", updatedAt: "",
  })
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const data = await portalRequest<{ id: string }>("/alliance/brand-topics", { method: "POST", body: JSON.stringify(item) })
      toast({ title: "专题已创建" })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold">新建品牌专题</h1>
      </div>
