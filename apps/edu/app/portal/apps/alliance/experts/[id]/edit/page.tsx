    ...
    Promise.all([
      ...
      .then(([expert, ents]) => {
        setItem({
          ...
          partnerSource: expert.partnerSource || "cooperation",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                <Label>年龄</Label>
                <Input type="number" value={item.age ?? ""} onChange={(e) => setField("age", e.target.value ? Number(e.target.value) : undefined)} />
              </div>
              <div className="grid gap-2">
                <Label>所在城市</Label>
              ...
              <div className="grid gap-2">
                <Label>职称/职位</Label>
                <Input value={item.title} onChange={(e) => setField("title", e.target.value)} placeholder="如：高级工程师" />
              </div>
              <div className="grid gap-2">
                <Label>任职岗位</Label>
              ...
              <div className="grid gap-2">
                <Label>从业年限</Label>
                <Input type="number" value={item.experienceYears ?? ""} onChange={(e) => setField("experienceYears", e.target.value ? Number(e.target.value) : undefined)} />
              </div>
              <div className="grid gap-2">
                <Label>教育背景</Label>
                <Input value={item.education} onChange={(e) => setField("education", e.target.value)} placeholder="如：XX大学 硕士" />
              </div>
              <div className="grid gap-2">
                <Label>行业方向</Label>
                <Input value={item.industry} onChange={(e) => setField("industry", e.target.value)} placeholder="如：智能制造" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader><CardTitle>擅长领域</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <div className="flex flex-wrap gap-2">
                {item.specialties.map((s) => (
                  <span key={s} className="inline-flex items-center gap-1 rounded bg-blue-50 text-blue-600 px-2 py-0.5 text-xs">
                    {s}
                    <button onClick={() => setField("specialties", item.specialties.filter((x) => x !== s))}>
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ))}
              ...
              <div className="flex gap-2">
                <Input value={specialtyInput} onChange={(e) => setSpecialtyInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addSpecialty())} placeholder="输入擅长领域后回车添加" className="h-8 text-xs" />
                <Button type="button" variant="outline" size="sm" onClick={addSpecialty} className="h-8 text-xs"><Plus className="h-3 w-3 mr-1" />添加</Button>
              </div>
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>专家简介</CardTitle></CardHeader>
            <CardContent>
              <Textarea value={item.introduction} onChange={(e) => setField("introduction", e.target.value)} rows={4} />
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>从业经历</CardTitle></CardHeader>
            <CardContent>
              <Textarea value={item.workExperience} onChange={(e) => setField("workExperience", e.target.value)} rows={4} />
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>资质荣誉（佐证材料）</CardTitle></CardHeader>
            <CardContent>
              <ImageListUpload label="佐证材料" value={item.attachments} onChange={(v) => setField("attachments", v)} multiple placeholder="上传附件或输入 URL" />
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader><CardTitle>所属机构来源</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <div className="grid gap-2">
                <Label>来源</Label>
                <Select value={item.partnerSource} onValueChange={(v) => setField("partnerSource", v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              {item.partnerSource === "cooperation" ? (
                <div className="grid gap-2">
                  <Label>选择企业</Label>
                  <Select value={item.enterpriseId} onValueChange={(v) => setField("enterpriseId", v)}>
                    <SelectTrigger><SelectValue placeholder="选择合作企业" /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="grid gap-2">
                  <Label>机构名称</Label>
                  <Input value={item.organization} onChange={(e) => setField("organization", e.target.value)} placeholder="输入第三方机构名称" />
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <MultiSelect options={SECONDARY_COLLEGES} value={item.secondaryColleges} onChange={(v) => setField("secondaryColleges", v)} placeholder="选择归属学院" />
            </CardContent>
          </Card>
