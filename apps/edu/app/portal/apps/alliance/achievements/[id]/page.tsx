...
interface RelatedRef { id: string; name: string }
...
export default function AllianceAchievementDetailPage() {
  ...
  const [pickDialog, setPickDialog] = useState<{ open: boolean; kind: "positions" | "scenes" | "courses"; selected?: string }>({ open: false, kind: "positions" })

  const loadData = () => {
    if (!tenantId || !id) return
    Promise.all([
      portalRequest<AllianceAchievement>(`/alliance/achievements/${id}`),
      portalRequest<{ items: RelatedRef[] }>("/career/positions?limit=1000").catch(() => ({ items: [] })),
  ...
  const saveRelated = async (key: "relatedPositions" | "relatedScenes" | "relatedCourses", items: RelatedRef[]) => {
    if (!achievement) return
    setSaving(true)
    try {
      const updated = { ...achievement, [key]: items } as AllianceAchievement
      await portalRequest(`/alliance/achievements/${id}`, { method: "PUT", body: JSON.stringify(updated) })
  ...
  const currentItems = (): RelatedRef[] => {
    const key = pickDialog.kind === "positions" ? "relatedPositions" : pickDialog.kind === "scenes" ? "relatedScenes" : "relatedCourses"
  ...
  const removeItem = async (key: "relatedPositions" | "relatedScenes" | "relatedCourses", refId: string) => {
  ...
  const renderRelated = (key: "relatedPositions" | "relatedScenes" | "relatedCourses", kind: "positions" | "scenes" | "courses", label: string) => {
    const items: RelatedRef[] = (achievement?.[key] as any) || []
    return (
      <div className="space-y-4">
        <div className="flex justify-end">
          <Button size="sm" onClick={() => setPickDialog({ open: true, kind })} disabled={optionsFor(kind).length === 0}>
            <Plus className="h-4 w-4 mr-1" />添加{label}
          </Button>
        </div>
        {items.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {items.map((ref) => (
              <div key={ref.id} className="inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm bg-muted/20">
                <span>{ref.name}</span>
                <button className="text-red-500 hover:text-red-700" onClick={() => removeItem(key, ref.id)}>
  ...
  const tabs = [
        ...
        <div className="grid grid-cols-2 gap-6">
          <Card><CardHeader><CardTitle>基础信息</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm">
              ...
              <p><span className="text-muted-foreground">前台展示：</span>{achievement?.isPublic ? "是" : "否"}</p>
              <p><span className="text-muted-foreground">浏览量：</span>{achievement?.viewCount || 0}</p>
              <p><span className="text-muted-foreground">创建人：</span>{achievement?.createdBy || "-"}</p>
            </CardContent></Card>
          <Card><CardHeader><CardTitle>引用来源</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm">
              ...
              {achievement?.ownerPersons && (achievement.ownerPersons as any[])?.length > 0 && (
                <p><span className="text-muted-foreground">成果归属人：</span>
                  {(achievement.ownerPersons as any[]).map((p: any) => <Badge key={p} variant="secondary" className="mr-1">{p.name || p}</Badge>)}
                </p>
              )}
              {achievement?.coBuilders && (achievement.coBuilders as any[])?.length > 0 && (
                <p><span className="text-muted-foreground">成果共建人：</span>
                  {(achievement.coBuilders as any[]).map((p: any) => <Badge key={p} variant="secondary" className="mr-1">{p.name || p}</Badge>)}
                </p>
              )}
            </CardContent></Card>
          {achievement?.coverImage && (
            <Card><CardHeader><CardTitle>成果封面</CardTitle></CardHeader>
              <CardContent>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={achievement.coverImage} alt={achievement.title} className="w-full max-h-48 object-cover rounded-lg" />
              </CardContent></Card>
          )}
          {achievement?.description && (
            <Card className="col-span-2"><CardHeader><CardTitle>成果简介</CardTitle></CardHeader>
              <CardContent><p className="text-sm whitespace-pre-wrap">{achievement.description}</p></CardContent></Card>
          )}
        </div>
      ),
    },
    {
      key: "attachments", label: "成果佐证材料", badge: (achievement?.attachments || []).length,
      content: (
    ...
    {
      key: "positions", label: "关联职业岗位", badge: ((achievement as any)?.relatedPositions || []).length,
      content: renderRelated("relatedPositions", "positions", "岗位"),
    },
    {
      key: "scenes", label: "关联实践场景", badge: ((achievement as any)?.relatedScenes || []).length,
      content: renderRelated("relatedScenes", "scenes", "场景"),
    },
    {
      key: "courses", label: "关联数字课程", badge: ((achievement as any)?.relatedCourses || []).length,
      content: renderRelated("relatedCourses", "courses", "课程"),
    },
  ]

  const pickLabel = pickDialog.kind === "positions" ? "岗位" : pickDialog.kind === "scenes" ? "场景" : "课程"
  ...
  return (
    <>
      ...
      <Dialog open={pickDialog.open} onOpenChange={(o) => !o && setPickDialog({ open: false, kind: pickDialog.kind })}>
        <DialogContent>
          <DialogHeader><DialogTitle>添加关联{pickLabel}</DialogTitle></DialogHeader>
          <div className="max-h-[50vh] overflow-y-auto space-y-1">
            ...
            {pickOptions.length === 0 && <p className="text-center py-6 text-sm text-muted-foreground">暂无可选{pickLabel}</p>}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setPickDialog({ open: false, kind: pickDialog.kind })}>取消</Button>
            <Button onClick={addItem} disabled={saving || !pickDialog.selected}>{saving ? "保存中..." : "添加"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
