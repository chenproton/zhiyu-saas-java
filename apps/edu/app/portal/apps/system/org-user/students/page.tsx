...
export default function StudentsPage() {
  ...
  return (
    <>
      ...
      importConfig={{ importType: "students", entityLabel: "学生", templateFileName: "学生批量导入模板.xlsx" }}
      colSpan={6}
      renderTableHeader={() => (
        <>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                toast({ title: "批量毕业成功", description: `已将 ${selectedIds.length} 名学生状态改为毕业` })
                await refetch()
              } catch (err) {
      ...
      onOpenCreate={() => { setSelectedStudent(null); resetForm(); setIsDialogOpen(true) }}
      onOpenEdit={(student) => {
      ...
      editDialogDescription={selectedStudent ? "修改学生基本信息与班级归属" : "填写学生基本信息，并关联到真实班级"}
      renderForm={() => (
        <>
