...
export default function MajorsPage() {
  ...
  return (
    <PortalCrudPage
      title={t('专业管理')}
      description={t('管理教育专业，可为专业配置别名并启用/关闭')}
      entityLabel={t('专业')}
      searchPlaceholder={t('搜索专业代码、名称或别名...')}
      createButtonLabel={t('新建专业')}
      items={majors}
      loading={loading}
      ...
      importConfig={{
        importType: 'majors',
        entityLabel: t('专业'),
        templateFileName: t('专业批量导入模板.xlsx'),
      }}
      colSpan={6}
      renderTableHeader={() => (
        <>
          <TableHead className="w-28">{t('专业代码')}</TableHead>
          <TableHead className="w-32">{t('专业名称')}</TableHead>
          <TableHead className="hidden md:table-cell">{t('别名（备注）')}</TableHead>
      ...
      renderForm={(item, setItem) => {
        ...
        return (
          <>
            <FormFieldRow label={t('专业代码')} required>
              <Input
                placeholder={t('如：CS101')}
                value={item.code}
                onChange={(e) => set({ code: e.target.value })}
                disabled={!!item.id}
              />
            </FormFieldRow>
            <FormFieldRow label={t('专业名称')} required>
              <Input
                placeholder={t('如：计算机科学与技术')}
                value={item.name}
                onChange={(e) => set({ name: e.target.value })}
              />
            </FormFieldRow>
            <FormFieldRow label={t('别名（备注）')}>
              <Input
                placeholder={t('输入专业别名或备注')}
                value={item.alias || ''}
      ...
      getDeleteDescription={(item) => (
        <>{t('确定要删除专业「{name}」（{code}）吗？此操作不可撤销。', { name: item.name, code: item.code })}</>
      )}
      onSave={async (item, isEdit) => {
        ...
        if (isEdit) {
          ...
          toast({ title: t('保存成功'), description: t('专业信息已更新') })
        } else {
          ...
          toast({ title: t('创建成功'), description: t('新专业已添加') })
        }
      }}
      onDelete={async (item) => {
