...
export function AccountInfoForm() {
  ...
  const handleSave = async () => {
    ...
    if (!trimmed) {
      toast({ variant: 'destructive', title: '保存失败', description: '姓名不能为空' })
      return
    }
    setSaving(true)
    try {
      ...
      setName(trimmed)
      await refresh()
      toast({ title: '保存成功', description: '姓名已更新' })
    } catch (e) {
      toast({
        variant: 'destructive',
        title: '保存失败',
        description: e instanceof Error ? e.message : '更新姓名失败',
      })
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div className="space-y-4">
      <FormFieldGrid>
        <FormFieldRow label="用户ID" labelClassName="text-gray-700">
          <Input value={user?.id || '—'} disabled className="bg-gray-50 border-gray-100" />
        </FormFieldRow>
        <FormFieldRow label="用户名（登录账号）" labelClassName="text-gray-700">
          <Input value={user?.username || '—'} disabled className="bg-gray-50 border-gray-100" />
        </FormFieldRow>
        <FormFieldRow label="姓名" labelClassName="text-gray-700">
          <Input
            value={name}
