...
import {
  User, Search, ChevronLeft, GraduationCap,
  Users, Eye, BarChart3, FileText, Target, Loader2,
} from "lucide-react"
...
export function TeacherPortraitsTab() {
  ...
  useEffect(() => {
    let cancelled = false
    jobAbilityResultApi.list({ limit: 200 })
      .then((res) => { if (!cancelled) { setAllResults(res.items || []); setLoading(false) } })
      .catch(() => { if (!cancelled) { setAllResults([]); setLoading(false) } })
  ...
  const groupedByClass = useMemo(() => {
    ...
    students.forEach((s) => {
      ...
      if (q && !major.toLowerCase().includes(q) && !cls.toLowerCase().includes(q) && !s.name.toLowerCase().includes(q) && !s.studentNo.toLowerCase().includes(q)) return
            ...
            <div className="space-y-3 max-h-[60vh] overflow-auto pr-1">
              {filteredStudents.map((student) => {
                ...
                return (
                  ...
                  >
                    <div className={`absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b ${getGradeBg(student.grade)}`} />
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-1.5">
                              <Button
                                ...
                                onClick={(e) => { e.stopPropagation(); setSelectedUserId(student.userId) }}
                              >
                          ...
                          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            ...
                            <span className="text-[11px] text-gray-400">|</span>
                            <span className="text-xs text-gray-500">达成率 {student.achievementRate.toFixed(1)}%</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <Table>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell className="text-sm">{activeStudent?.achievementRate.toFixed(1) ?? "-"}%</TableCell>
            </TableRow>
            <TableRow>
              <TableCell className="text-sm font-medium">认定等级</TableCell>
              <TableCell><Badge className={`text-xs ${getGradeColor(activeStudent?.grade)}`}>{activeStudent?.grade || "-"}</Badge></TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </DialogContent>
