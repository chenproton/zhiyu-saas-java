...
export default function AlliancePublicEnterprisesPage() {
  ...
  const tabs = useMemo(
    () => [
      { value: 'all', label: t('全部企业'), count: items.length },
      ...RATING_TABS.map((tab) => ({
        value: tab.value,
        label: t(tab.label),
  ...
  return (
    <PublicListShell
      ...
      placeholder={t('搜索企业名称、行业或地区...')}
      loading={loading}
    >
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-[#94a3b8] bg-white rounded-2xl border border-[#e7e5e4] shadow-sm">
          <Building2 className="h-12 w-12 mx-auto mb-4 opacity-30" />
          <div className="text-[15px] font-medium text-[#475569]">{t('暂无合作企业')}</div>
          <div className="text-[13px] mt-1">{t('发布后的合作企业会展示在这里')}</div>
        </div>
      ) : (
