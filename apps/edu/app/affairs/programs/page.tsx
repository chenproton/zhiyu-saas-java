...
function mapProgram(backend: any) {
  return { ...(backend as TrainingProgram), creatorId: backend.createdBy || "", coCreatorIds: backend.collaborators || [] } as any
}
function mapBatch(backend: any) { return { id: backend.id, name: backend.name, workflowId: backend.workflowId } }
...
export default function ProgramsPage() {
  ...
  return (
    <ContentListPage<TrainingProgram & { creatorId: string; coCreatorIds: string[] }>
      ...
      statusFilterOptions={[
        { value: "draft", label: "草稿" }, { value: "pending", label: "审批中" },
        { value: "approved", label: "已通过" }, { value: "rejected", label: "已驳回" },
        { value: "published", label: "已发布" }, { value: "archived", label: "已归档" },
      ]}
      mapItem={mapProgram} mapBatch={mapBatch}
      createPayload={() => ({ name: "新建人培方案", entryYear: new Date().getFullYear(), level: "本科", duration: 4, totalCredits: 0, status: "draft", collaborators: [] })}
      renderList={(props: any) => {
        const { items, selectedIds, onSelectId, onSelectAll, onClone, onDelete, onSubmitApproval, onWithdrawApproval, onPublish, onUnpublish, onArchive, onInviteCoBuild, batchMap } = props
        return (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead><tr className="border-b">
                  ...
                  <th className="sticky right-0 w-[180px] bg-white px-2 py-2 text-right text-xs font-medium text-muted-foreground">操作</th>
                </tr></thead>
                <tbody>
                  {items.length === 0 ? (
                    ...
                    <tr key={item.id} className="border-t hover:bg-muted/30 group">
                      <td className="px-2 py-2"><input type="checkbox" checked={selectedIds?.includes(item.id)} onChange={() => onSelectId?.(item.id)} /></td>
                      <td className="px-2 py-2"><div className="font-medium text-sm">{item.name}</div>{item.code && <div className="text-xs text-muted-foreground">{item.code}</div>}</td>
