...
export const ProgramCoursesTab = forwardRef(function ProgramCoursesTab({ programId }: { programId: string }, ref: any) {
  ...
  const loadCourses = useCallback(async () => {
    try {
      ...
      const loadedRows: CourseRow[] = res.items.map((c) => ({
        ...
        courseId: c.courseId || "", positionId: c.positionId || "",
      }))
      const posIds = [...new Set(loadedRows.filter((r) => r.positionId).map((r) => r.positionId))]
      for (const pid of posIds) {
        try { const s = await scenarioApi.list({ careerPositionId: pid, status: "published", limit: 200 }); setPositionScenariosMap((prev) => ({ ...prev, [pid]: s.items })) } catch { /* ignore */ }
      }
      const grouped = new Map<string, CourseRow[]>()
      const regular: CourseRow[] = []
      loadedRows.forEach((r) => { if (r.linkType === "position" && r.positionId) { const g = grouped.get(r.positionId) || []; g.push(r); grouped.set(r.positionId, g) } else { regular.push(r) } })
      const displayRows: CourseRow[] = []
      grouped.forEach((v, pid) => { displayRows.push({ ...v[0], key: `pos-${pid}-${Date.now()}`, linkType: "position", positionId: pid }) })
  ...
  const handlePositionChange = async (rowKey: string, newPositionId: string) => {
    if (!newPositionId || newPositionId === "none") { updateRow(rowKey, { positionId: "" }); return }
  ...
  const totalCredits = useMemo(() => { let s = 0; rows.forEach((r) => { s += (r.credits || 0) * (r.linkType === "position" && r.positionId ? (positionScenariosMap[r.positionId] || []).length || 1 : 1) }); return s }, [rows, positionScenariosMap])
    ...
    <div className="rounded-lg border bg-white px-4 py-3 space-y-3">
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {loading ? (<TableRow><TableCell colSpan={6} className="h-24 text-center text-muted-foreground">加载中...</TableCell></TableRow>)
              : rows.length === 0 ? (<TableRow><TableCell colSpan={6} className="h-24 text-center text-muted-foreground">暂无，点击「添加岗位/课程」开始设置</TableCell></TableRow>)
                : rows.map((r) => {
                  ...
                  return (
                    <TableRow key={r.key}>
                        ...
                        {r.linkType === "position" ? (
                          <div className="space-y-1">
                            <div className="flex items-center gap-1">
                              <Select value={r.linkType} onValueChange={(v) => { updateRow(r.key, { linkType: v as LinkType, positionId: "", courseId: "" }) }}>
                                <SelectTrigger className="h-8 w-[80px]"><SelectValue /></SelectTrigger>
                                <SelectContent><SelectItem value="none">未关联</SelectItem><SelectItem value="position">岗位</SelectItem><SelectItem value="course">体系课</SelectItem></SelectContent>
                              </Select>
                              <ComboboxSelect value={r.positionId} onChange={(v) => handlePositionChange(r.key, v || "")} options={positionOpts} placeholder="搜索岗位..." emptyText="未找到岗位" className="flex-1" />
                            </div>
                            {r.positionId && (
                              <div className="text-xs text-muted-foreground pl-1">
                                {loadingPosScen[r.positionId] ? "加载中..." : posScenarios.length > 0 ? `包含 ${posScenarios.length} 个场景：${posScenarios.map((s) => s.name).join("、")}` : "该岗位下暂无已发布场景"}
                              </div>
                            )}
                          </div>
                        ) : r.linkType === "course" ? (
                          <div className="flex items-center gap-1">
                            <Select value={r.linkType} onValueChange={(v) => updateRow(r.key, { linkType: v as LinkType, courseId: "", positionId: "" })}>
                              <SelectTrigger className="h-8 w-[80px]"><SelectValue /></SelectTrigger>
                              <SelectContent><SelectItem value="none">未关联</SelectItem><SelectItem value="position">岗位</SelectItem><SelectItem value="course">体系课</SelectItem></SelectContent>
                            </Select>
                            <ComboboxSelect value={r.courseId} onChange={(v) => { const cid = v || ""; const course = systemCourses.find((c) => c.id === cid); updateRow(r.key, { courseId: cid, name: course ? course.name : r.name, code: course ? (course.code || "") : r.code, hours: course ? (course.onlineHours || 0) : r.hours }) }} options={courseOpts} placeholder="搜索体系课..." emptyText="未找到体系课" className="flex-1" />
                          </div>
                        ) : (
                          <div className="flex items-center gap-1">
                            <Select value={r.linkType} onValueChange={(v) => updateRow(r.key, { linkType: v as LinkType, positionId: "", courseId: "" })}>
                      ...
                      <TableCell><span className="text-sm text-muted-foreground">{r.code || "-"}</span></TableCell>
                      <TableCell><Input className="h-8" type="number" min={0} step="0.5" value={r.credits} onChange={(e) => updateRow(r.key, { credits: Number(e.target.value) })} /></TableCell>
                      <TableCell><Input className="h-8" type="number" min={0} value={r.hours} onChange={(e) => updateRow(r.key, { hours: Number(e.target.value) })} /></TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-red-500 hover:text-red-600" onClick={() => removeRow(r.key)}><Trash2 className="h-3 w-3" /></Button>
                      </TableCell>
                    </TableRow>
                  )
                })}
