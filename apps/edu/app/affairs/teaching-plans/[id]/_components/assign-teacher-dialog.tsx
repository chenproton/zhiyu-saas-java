...
export function AssignTeacherDialog({ open, onOpenChange, entry, onSaved }: AssignTeacherDialogProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "保存失败", description: err.message || "指定教师失败" })
    } finally {
      setSaving(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            为「{entry?.courseName || ""}」指定授课教师
          </DialogDescription>
        </DialogHeader>
        <FieldGroup className="py-4">
          <Field>
            <FieldLabel>师资类型</FieldLabel>
            <Select value={teacherType || "none"} onValueChange={(v) => setTeacherType(v === "none" ? "" : v)}>
              <SelectTrigger>
