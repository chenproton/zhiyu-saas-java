...
export function ScheduleFormDialog({
  ...
  const handleSubmit = async () => {
    ...
    if (!courseName || periods.length === 0 || !start || !end || start > end) {
      toast({ variant: "destructive", title: "请完善表单", description: "课程名称、节次、起止周为必填，且起始周不能大于结束周" })
      return
    }
    setSaving(true)
    setConflicts([])
    try {
      ...
      if (err instanceof ScheduleConflictError) {
        ...
        toast({ variant: "destructive", title: "保存失败", description: err.message || "保存排课失败" })
      }
    } finally {
      setSaving(false)
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "删除失败", description: err.message || "删除排课失败" })
    } finally {
      setDeleting(false)
    }
  }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
          ...
          <FieldGroup className="py-2">
            <Field>
              ...
              <div className="flex items-center gap-2">
                ...
                {isScene && (
                  <span className="shrink-0 rounded-full bg-orange-100 px-2 py-0.5 text-xs font-medium text-orange-600">场景</span>
                )}
              </div>
            </Field>
            {teacherName && (
            ...
            <Field>
              ...
              {periodSlots.length === 0 ? (
                <p className="text-xs text-muted-foreground">暂无节次，请先在「教务基础配置」中创建</p>
              ) : (
                  ...
                  {[...periodSlots]
                    ...
                    .map((s) => (
                      <label key={s.id} className="flex cursor-pointer items-center gap-2 text-sm">
                        <Checkbox checked={periods.includes(s.name)} onCheckedChange={() => togglePeriod(s.name)} />
                        {s.name}
                      </label>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field>
                ...
                <Input type="number" min={1} value={startWeek} onChange={(e) => setStartWeek(e.target.value)} />
              </Field>
              <Field>
                <FieldLabel>结束周 *</FieldLabel>
                <Input type="number" min={1} value={endWeek} onChange={(e) => setEndWeek(e.target.value)} />
              </Field>
            </div>
            <Field>
            ...
            {conflicts.length > 0 && (
              <div className="space-y-1.5 rounded-md border border-red-200 bg-red-50 p-3">
                <div className="text-sm font-medium text-red-700">排课冲突（{conflicts.length} 项），请调整后重试</div>
                {conflicts.map((c, idx) => (
                  <div key={`${c.entryId}-${c.kind}-${idx}`} className="text-xs leading-5 text-red-600">
                    <span className="font-medium">【{CONFLICT_KIND_LABELS[c.kind] || "冲突"}】</span>
                    与「{c.courseName}」
                    {c.teacherName ? `（${c.teacherName}）` : ""}
                    {c.className ? ` ${c.className}` : ""}
                    {c.venueName ? ` @${c.venueName}` : ""}
                    在{DAY_OPTIONS[c.dayOfWeek - 1]?.label || `周${c.dayOfWeek}`}
          ...
          <DialogFooter className="gap-2 sm:justify-between">
            <div>
              {editing && (
                <Button variant="destructive" onClick={() => setDeleteConfirmOpen(true)} disabled={saving || deleting}>
                  删除排课
                </Button>
              )}
            </div>
