...
function SchedulingPageInner() {
  ...
  const loadPlanDetail = useCallback(async (id: string) => {
    if (!id) { setPlanDetail(null); return }
    setLoadingPlan(true)
    try {
  ...
  useEffect(() => { ;(async () => { await loadPlans() })() }, [loadPlans])
  useEffect(() => { ;(async () => { await loadPlanDetail(planId) })() }, [planId, loadPlanDetail])
  ...
  return (
    <div className="space-y-6">
      <PageHeaderCard
        title="排课管理"
        description={selectedPlan ? `当前教学计划：${selectedPlan.programName} · ${selectedPlan.termName} · ${selectedPlan.majorName || ""} ${selectedPlan.entryYear}级` : "选择教学计划开始排课，发布后学生/教师工作台可见"}
        actions={
          <Select value={planId} onValueChange={setPlanId}>
            ...
            <SelectContent>
              {plans.length === 0 ? (
                <SelectItem value="__empty" disabled>暂无可用的已确认教学计划，请先在「教学计划」中生成并确认</SelectItem>
              ) : (
                plans.map((p) => (
                  <SelectItem key={p.id} value={p.id}>{p.programName} · {p.termName}{p.majorName ? ` · ${p.majorName}` : ""}</SelectItem>
                ))
              )}
            </SelectContent>
          </Select>
          ...
          {STEPS.map((s, idx) => {
            ...
            return (
              <button key={s.id} onClick={() => setStep(s.id)} className={cn(
                ...
                <span className={cn("flex h-5 w-5 items-center justify-center rounded-full text-xs", isActive ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500")}>{idx + 1}</span>
      ...
      {step === "grid" && !selectedPlan && (
        <div className="rounded-lg border bg-white py-16 text-center text-sm text-muted-foreground">请先在顶部选择已确认的教学计划</div>
      )}
      {step === "grid" && selectedPlan && planDetail && (
        <ScheduleGridTab plan={selectedPlan} planEntries={planDetail.entries} onPlanChanged={() => loadPlanDetail(planId)} />
      )}
      {step === "timetable" && selectedPlan && (
        <TimetableViewTab term={{ id: selectedPlan.termId, name: selectedPlan.termName, startDate: "", endDate: "", weeksCount: 0, isCurrent: false, createdAt: "" } as any} />
      )}
    </div>
  )
}
