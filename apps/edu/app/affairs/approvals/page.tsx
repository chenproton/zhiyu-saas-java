...
export default function AffairsApprovalsPage() {
  ...
  const columns: ApprovalColumn<ApprovalView>[] = [
    ...
    { header: "所属批次组", cell: (i) => <span className="text-sm text-gray-600">{i.batchName || "-"}</span> },
    { header: "创建人", className: "text-sm text-gray-600", cell: (i) => getName(i.submitterId) },
    { header: "提交日期", cell: (i) => <span className="text-sm text-gray-600">{i.submittedAt}</span> },
  ...
  const mapRecord = (a: any): ApprovalView => {
    ...
    return {
      id: a.id, programId: a.targetId,
      programName: program?.name || a.targetId,
      batchName: batch?.name, submitterId: a.submitterId, status: a.status,
      submittedAt: new Date(a.createdAt).toLocaleDateString(),
      stepInfo: getStepInfo(a), history: a.history,
    }
  }
  ...
  return (
    <ApprovalListPage<ApprovalView>
      ...
      records={records} loading={loading} getStepInfo={getStepInfo}
      onApprove={approve} onReject={reject}
      onBatchApprove={batchApprove} onBatchReject={batchReject}
      mapRecord={mapRecord}
