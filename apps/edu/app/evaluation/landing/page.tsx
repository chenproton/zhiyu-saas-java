  ...
  return (
    <Link href={`/evaluation/landing/banks/${bank.id}`} className="group block no-underline text-inherit">
      <div className="bg-white rounded-2xl border border-[#e7e5e4] overflow-hidden hover:shadow-[0_8px_28px_rgba(0,0,0,0.1)] hover:border-purple-200 hover:-translate-y-1 transition-all h-full flex flex-col shadow-[0_2px_6px_rgba(0,0,0,0.04)]">
        <div className="h-[110px] flex items-center justify-center shrink-0 relative" style={{ background: coverGradients[index % coverGradients.length] }}>
          <Library className="w-12 h-12 text-white/80" />
          <span className="absolute top-3 right-3 bg-white/20 backdrop-blur-sm text-white px-2.5 py-1 rounded-full text-[11px] font-medium border border-white/10">v{bank.version}</span>
        </div>
        <div className="p-5 flex-1 flex flex-col">
          ...
          <div className="flex items-center justify-between text-[11px] text-slate-400 border-t border-slate-50 pt-3">
            ...
            <span className="text-purple-500 group-hover:text-purple-600 font-medium">查看详情 →</span>
          </div>
        </div>
      </div>
    </Link>
...
function ExamCard({ exam, index }: { exam: Exam; index: number }) {
  return (
    <Link href={`/evaluation/landing/exams/${exam.id}`} className="group block no-underline text-inherit">
      <div className="bg-white rounded-2xl border border-[#e7e5e4] overflow-hidden hover:shadow-[0_8px_28px_rgba(0,0,0,0.1)] hover:border-purple-200 hover:-translate-y-1 transition-all h-full flex flex-col shadow-[0_2px_6px_rgba(0,0,0,0.04)]">
        <div className="h-[110px] flex items-center justify-center shrink-0 relative" style={{ background: coverGradients[(index + 3) % coverGradients.length] }}>
          <ClipboardList className="w-12 h-12 text-white/80" />
          <span className="absolute top-3 right-3 bg-white/20 backdrop-blur-sm text-white px-2.5 py-1 rounded-full text-[11px] font-medium border border-white/10">{exam.duration} 分钟</span>
        </div>
        <div className="p-5 flex-1 flex flex-col">
...
export default function LandingHomePage() {
  ...
  const filteredBanks = useMemo(() => {
    let list = [...banks]
    if (keyword.trim()) { const k = keyword.trim().toLowerCase(); list = list.filter((b) => b.name.toLowerCase().includes(k) || (b.description || "").toLowerCase().includes(k)) }
    if (selectedBatch !== "全部") list = list.filter((b) => b.batchId && (batchNames.get(b.batchId) || b.batchId) === selectedBatch)
    switch (sort) {
  ...
  const filteredExams = useMemo(() => {
    let list = [...exams]
    if (keyword.trim()) { const k = keyword.trim().toLowerCase(); list = list.filter((e) => e.name.toLowerCase().includes(k) || (e.description || "").toLowerCase().includes(k)) }
    if (selectedBatch !== "全部") list = list.filter((e) => e.batchId && (batchNames.get(e.batchId) || e.batchId) === selectedBatch)
    switch (sort) {
      case "recent": list.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()); break
      case "update": list.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()); break
      default: list.sort((a, b) => a.name.localeCompare(b.name, "zh-CN")); break
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#faf5ff]">
      ...
      <div className="relative w-full pt-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-8 pb-14 pt-2 flex flex-col lg:flex-row justify-between items-start gap-10">
          <div className="flex-1 pt-4">
            ...
            <p className="text-[18px] text-white/80 mb-8 max-w-xl leading-relaxed">丰富题库资源与智能组卷工具，支持在线考试与自动评分，让教学测评更高效</p>
            <Button className="inline-flex items-center gap-2 bg-white text-purple-600 hover:bg-purple-50 hover:-translate-y-0.5 px-8 h-13 rounded-full text-[15px] font-semibold shadow-xl shadow-purple-500/25 transition-all" onClick={() => listRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })}>浏览资源 <ChevronRight className="w-4 h-4" /></Button>
          </div>
          <div className="w-full lg:w-[420px] shrink-0 flex flex-col gap-5 pt-6">
            <div className="bg-white/10 backdrop-blur-xl border border-white/15 rounded-2xl p-7 text-white shadow-[0_12px_40px_rgba(0,0,0,0.25)]">
              <div className="text-[15px] font-bold text-white/90 mb-5 flex items-center gap-2"><span className="w-5 h-5 rounded-md bg-white/20 flex items-center justify-center">📊</span>平台统计</div>
              <div className="space-y-4">
                <div className="flex justify-between items-center"><span className="text-[14px] text-white/65">题库总数</span><span className="text-[26px] font-bold">{banks.length.toLocaleString()}</span></div>
                <hr className="border-white/8" />
                <div className="flex justify-between items-center"><span className="text-[14px] text-white/65">试卷总数</span><span className="text-[26px] font-bold">{exams.length.toLocaleString()}</span></div>
                <hr className="border-white/8" />
                <div className="flex justify-between items-center"><span className="text-[14px] text-white/65">题目总数</span><span className="text-[26px] font-bold">{totalQuestions.toLocaleString()}</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-[1400px] mx-auto px-8 -mt-12 relative z-20 w-full">
        <div className="bg-white rounded-2xl border border-[#e7e5e4] shadow-[0_12px_40px_rgba(0,0,0,0.08)] p-6 grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { icon: ClipboardList, value: exams.length, label: "试卷总数", gradient: "from-violet-500 to-violet-400" },
            { icon: FileText, value: totalQuestions, label: "题目总数", gradient: "from-fuchsia-500 to-fuchsia-400" },
            { icon: PlayCircle, value: exams.length, label: "可参与考试", gradient: "from-pink-500 to-pink-400" },
          ].map((s, i) => (
            <div key={i} className="flex items-center gap-4 p-4 rounded-xl transition-all hover:-translate-y-0.5 hover:shadow-lg hover:bg-[#faf5ff] cursor-default group">
      ...
      <main ref={listRef} className="max-w-[1400px] mx-auto px-8 py-8 w-full flex-1">
        ...
        <div className="text-[13px] text-[#64748b] mb-6"><span className="inline-flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-purple-400" />当前共展示 <b className="text-purple-600">{filteredBanks.length + filteredExams.length}</b> 个资源</span></div>

        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">{Array.from({ length: 8 }).map((_, i) => (<div key={i} className="bg-white rounded-2xl border border-[#e7e5e4] h-[340px] animate-pulse shadow-sm" />))}</div>
