...
import {
  ArrowLeft, Library, FileText, Clock, Layers, Search, Loader2,
  HelpCircle, ListChecks, PenTool, AlertCircle, CheckSquare, Type,
} from "lucide-react"
...
const questionTypeIcons: Record<string, React.ComponentType<any>> = {
  single: ListChecks, multiple: CheckSquare, judge: AlertCircle,
  fill: PenTool, essay: FileText, short_answer: Type,
}
    ...
    return (
      <div className="min-h-screen flex flex-col bg-[#f8fafc]">
        <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
          ...
          <Link href="/evaluation/landing" className="text-blue-600 hover:text-blue-700 mt-3 text-sm font-medium">返回测评首页</Link>
        </div>
        <PlatformFooter />
      </div>
    )
      ...
      <div className="bg-white border-b border-slate-200 shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
        <div className="max-w-[1400px] mx-auto px-6 py-5">
          <div className="flex items-center gap-2 mb-5 text-sm text-slate-500">
            ...
            <Link href="/evaluation/landing" className="hover:text-blue-600 transition-colors">测评首页</Link>
            <span className="text-slate-300">/</span>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.04)] w-full">
                <div className="flex flex-col sm:flex-row gap-6 p-6">
                  ...
                  >
                    ...
                    {!bank.coverImage && (
                      <Library className="w-16 h-16 text-white/90 relative z-10" style={{ filter: "drop-shadow(0 2px 6px rgba(0,0,0,0.2))" }} />
                    )}
                  ...
                  <div className="flex-1 flex flex-col min-w-0">
                    <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                      <div className="flex items-center gap-2.5 flex-wrap min-w-0">
                        <h1 className="text-[26px] font-bold text-slate-900 truncate">{bank.name}</h1>
                    ...
                    <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 text-xs text-slate-400 mb-3">
                      {bank.creatorName && <span className="flex items-center gap-1.5">创建人：{bank.creatorName}</span>}
                      <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> 更新于 {formatDate(bank.updatedAt)}</span>
                      <span className="flex items-center gap-1.5"><Layers className="w-3.5 h-3.5" /> {bank.questionCount} 题</span>
                    </div>

                    {bank.description && (
                      <p className="text-sm text-slate-600 leading-relaxed mb-4 line-clamp-3">{bank.description}</p>
                    )}
                      ...
                      {Object.entries(typeCounts).length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {Object.entries(typeCounts).map(([t, count]) => (
                            <span key={t} className={`px-2 py-0.5 rounded-full text-[11px] font-medium border ${questionTypeColors[t] || "bg-slate-50 text-slate-500 border-slate-100"}`}>
                ...
                <div className="p-5 space-y-3">
                  {Object.entries(typeCounts).map(([type, count]) => (
                    <div key={type} className="flex items-center justify-between py-2 border-b border-slate-50">
                      <span className="text-sm text-slate-500">{questionTypeLabels[type] || type}</span>
            ...
            {questions.length === 0 ? (
              ...
              <div className="space-y-3">
                {filteredQuestions.map((q, idx) => {
                  ...
                  return (
                    <div key={q.id} className="group bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover:border-blue-200 transition-all">
                      <div className="p-5">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap mb-2">
                              <span className={`text-[11px] px-2 py-0.5 rounded-full font-medium border ${questionTypeColors[q.type] || "bg-slate-50 text-slate-500 border-slate-100"}`}>
                            ...
                            {q.options && q.options.length > 0 && (
                              <div className="space-y-1 mb-2">
                                {q.options.map((opt, oi) => (
                                  <div key={oi} className="text-xs text-slate-500 flex items-center gap-2 pl-1">
                            ...
                            {(showAllAnswers || q.type !== "essay") && <AnswerPreview question={q} />}
                          </div>
                        </div>
                      </div>
                    </div>
