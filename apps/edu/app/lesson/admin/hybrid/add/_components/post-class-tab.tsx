...
const EVAL_POINT_TEMPLATES: Array<{
  id: string
  name: string
  desc: string
  weight: number
...
function RandomQuizPanel({
  config,
  setConfig,
}: {
  config: TaskEvaluationConfig
  setConfig: (c: TaskEvaluationConfig) => void
}) {
...
function PeerReviewPanel({
  ...
  const toggleStep = (id: string) => {
    setConfig({
      ...config,
      reviewSteps: (config.reviewSteps || []).map((s) =>
    ...
    setConfig({
      ...
      reviewSteps: [
        ...(config.reviewSteps || []),
        {
          ...
          desc: newStep.desc,
          enabled: true,
          subjectType: newStep.subjectType,
  ...
  const totalWeight = (config.reviewSteps || []).reduce(
    (sum, s) => sum + (s.enabled ? s.weight : 0),
    0,
  )
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="col-span-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewStep({ ...newStep, weight: Math.max(0, parseInt(e.target.value) || 0) })
                }
                className="mt-1 text-sm"
                min={0}
                max={100}
            ...
            <div key={s.id} className="flex items-center justify-between border rounded-lg p-3">
              ...
              <div className="flex items-center gap-2">
                <Input
                  type="number"
                  value={s.weight}
                  onChange={(e) => updateWeight(s.id, parseInt(e.target.value) || 0)}
        ...
        <CardContent className="pt-6 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                将学生本课程的全过程考核数据（出勤、作业、测验、互评、课堂表现等）统一归档生成最终成绩册。
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <Card>
              <CardContent className="p-3 text-center">
                <p className="text-lg font-bold">—</p>
                <p className="text-xs text-muted-foreground">作业/测验</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-3 text-center">
                <p className="text-lg font-bold">—</p>
                <p className="text-xs text-muted-foreground">互评活动</p>
              </CardContent>
            </Card>
          </div>
          <Button
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="random_quiz" className="w-full">
        <TabsList className="mb-3">
          <TabsTrigger value="random_quiz" className="text-xs">
            随机抽题
          </TabsTrigger>
          <TabsTrigger value="peer_review" className="text-xs">
            互评互判
          </TabsTrigger>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">评价维度模板</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <div className="flex justify-end">
        <Button onClick={() => toast({ title: '测评配置已保存' })}>
          <Save className="h-4 w-4 mr-1" />
          保存配置
        </Button>
      </div>
