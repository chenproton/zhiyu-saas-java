    ...
    <>
      ...
      <Dialog open={kpActionOpen} onOpenChange={setKpActionOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {kpActionMode === "add" ? "新增知识点" : kpActionMode === "clone" ? "克隆知识点" : "编辑知识点"}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4 py-4">
            <div>
              <Label>知识点名称</Label>
              <Input value={newKpForm.name} onChange={(e) => setNewKpForm({ ...newKpForm, name: e.target.value })} placeholder="输入知识点名称" className="mt-1.5" />
            </div>
            <div>
              <Label>描述</Label>
              <Textarea value={newKpForm.description} onChange={(e) => setNewKpForm({ ...newKpForm, description: e.target.value })} placeholder="输入知识点描述" className="mt-1.5" rows={3} />
            </div>
            <div>
              <Label>编码</Label>
              <Input value={newKpForm.code} disabled={kpActionMode !== "edit"} onChange={(e) => setNewKpForm({ ...newKpForm, code: e.target.value })} className={cn("mt-1.5", kpActionMode !== "edit" && "bg-gray-50")} />
      ...
      <Dialog open={glSelectOpen} onOpenChange={setGlSelectOpen}>
        <DialogContent className="sm:max-w-[800px] max-h-[80vh] h-[80vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>{glTargetKp ? `为「${glTargetKp.name}」选择颗粒课` : "选择颗粒课"}</DialogTitle>
          </DialogHeader>
          <div className="flex gap-4 flex-1 min-h-0 py-4">
            <div className="w-3/5 flex flex-col min-h-0 border rounded-xl p-3">
              <div className="relative mb-3">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                <Input value={glSearch} onChange={(e) => setGlSearch(e.target.value)} placeholder="搜索颗粒课名称或编码..." className="pl-9" />
              </div>
              <div className="flex-1 overflow-y-auto space-y-2 pr-1">
                {glFiltered.map((gl) => {
                  ...
                  return (
                    <div
                      key={gl.id}
                      className={cn("p-3 rounded-lg border cursor-pointer transition-all", isSelected ? "border-primary bg-primary/5" : "border-gray-200 hover:border-gray-300")}
                      onClick={() => {
                ...
                {glFiltered.length === 0 && (
                  <div className="text-center text-gray-400 py-8"><p className="text-sm">未找到匹配的颗粒课</p></div>
                )}
              </div>
            </div>
            <div className="w-2/5 border rounded-xl p-3 flex flex-col min-h-0">
              ...
              <div className="flex-1 overflow-y-auto space-y-2">
                {glSelectedIds.length === 0 ? (
                  <div className="text-center text-gray-400 py-8"><p className="text-xs">从左侧选择颗粒课</p></div>
                ) : (
      ...
      <Dialog open={kpDetailOpen} onOpenChange={setKpDetailOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>知识点详情</DialogTitle></DialogHeader>
          {detailKp && (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setKpDetailOpen(false)}>关闭</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
