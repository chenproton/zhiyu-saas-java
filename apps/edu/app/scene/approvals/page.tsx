...
export default function SceneApprovalsPage() {
  ...
  const columns: ApprovalColumn<ApprovalView>[] = [
    ...
    { header: "场景编码", cell: (i) => <span className="text-sm text-gray-600">{i.scenarioCode}</span> },
    { header: "版本", className: "text-center text-sm text-gray-600", cell: (i) => i.version },
    { header: "所属岗位", cell: (i) => <span className="text-sm text-gray-600">{i.positionName || "-"}</span> },
    { header: "所属批次分组", cell: (i) => <span className="text-sm text-gray-600">{i.batchName || "-"}</span> },
