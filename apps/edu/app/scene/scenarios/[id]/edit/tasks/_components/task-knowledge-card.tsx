...
interface TaskKnowledgeCardProps {
  ...
  setKpFormInitial: (
    v:
      | { name: string; description: string; code: string; granularLessonIds: string[] }
      | ((prev: {
          name: string
          description: string
          code: string
...
export function TaskKnowledgeCard({
  ...
  const [granularConfirm, setGranularConfirm] = useState<{ open: boolean; courseId: string }>({
    open: false,
    courseId: '',
  })
  ...
  const filteredKp = knowledgePoints.filter(
    (k) =>
      !kpSearch ||
      (k.name || '').includes(kpSearch) ||
      (k.description || '').includes(kpSearch) ||
      (k.code || '').includes(kpSearch),
  )
  ...
  const handleCreateGranularLesson = async (): Promise<string | undefined> => {
    ...
    try {
      ...
      setKpFormInitial((prev) => ({
        ...prev,
        granularLessonIds: [...prev.granularLessonIds, newCourseId],
      }))
      if (kpFormMode === 'edit' && kpFormTarget) {
        setKnowledgePoints(
          knowledgePoints.map((k) =>
  ...
  const handleToggleGlForKp = (glIds: string[]) => {
    ...
    setKnowledgePoints(
      knowledgePoints.map((k) =>
        k.id === glSelectTargetKp ? { ...k, granularLessons: glIds } : k,
      ),
    )
    updateState({ knowledgePoints: [...state.knowledgePoints] })
  }
  ...
  const detailKp = selectedKpForDetail
    ? knowledgePoints.find((k) => k.id === selectedKpForDetail)
    : null
  const detailGranularLessons =
    detailKp?.granularLessons
      ?.map((gid: string) => granularLessons.find((g) => g.id === gid))
      .filter(Boolean) || []

  const glTargetKp = glSelectTargetKp
    ? knowledgePoints.find((k) => k.id === glSelectTargetKp)
    : null
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex gap-4 flex-1 min-h-0">
                            ...
                            {isSelected ? (
                              ...
                              <>
                                <Button
                                  size="sm"
                                  className="h-6 text-[11px] px-2"
                                  onClick={() => handleReferenceKp(kp.id)}
                                >
                                  引用
                                </Button>
                                <Button
        ...
        <div className="w-2/5 border rounded-xl p-3 flex flex-col min-h-0">
          <p className="text-sm font-medium mb-3 text-gray-700">
            已选择知识点 ({state.knowledgePoints.length})
          </p>
            ...
            {state.knowledgePoints.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                {state.knowledgePoints.map((kpId) => {
                  ...
                  const kpGlNames =
                    kp.granularLessons
                      ?.map((gid: string) => granularLessons.find((g) => g.id === gid)?.name)
                      .filter(Boolean) || []
                  return (
                    <div
                      key={kpId}
                            ...
                            setKnowledgePoints(
                              knowledgePoints.map((k) =>
                                k.id === detailKp.id
                                  ? {
                                      ...k,
                                      granularLessons: [...(k.granularLessons || []), newCourseId],
                                    }
                                  : k,
                              ),
                            )
