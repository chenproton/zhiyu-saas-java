...
interface RandomDrawResourcePanelProps {
  ...
  setRdqActionTarget: (
    v: { id: string; name: string; description: string; answer: string } | null,
  ) => void
...
export function RandomDrawResourcePanel({
  ...
  const [rdqDrawMode, setRdqDrawMode] = useState<'random' | 'manual'>(
    (rdCfg.drawMode as 'random' | 'manual') ?? 'random',
  )
  ...
  const handleDeleteRdq = async (id: string) => {
    try {
      ...
      updateState({
        randomDrawSelectedIds: state.randomDrawSelectedIds.filter((sid: string) => sid !== id),
      })
      await loadQuestions()
    } catch {
      /* ignore */
  ...
  const selectedRdqList = state.randomDrawSelectedIds
          ...
          <div className="min-h-[560px] overflow-y-auto pr-1">
            {loading ? (
              ...
              <div className="text-center text-gray-400 py-8">
                ...
                <p className="text-sm">
                  {rdqSearch ? '未找到匹配的现场问答题' : '暂无现场问答题，请点击上方按钮新增'}
                </p>
              </div>
            ) : (
                  ...
                  {filteredRdq.map((q) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          <p className="text-xs text-gray-500 line-clamp-1" title={q.description}>
                            {q.description || '-'}
                          </p>
                        </td>
                        <td className="px-3 py-2">
              ...
              <div className="space-y-2">
                {selectedRdqList.map((q) => (
                  ...
                  >
                    ...
                    <Badge variant="outline" className="text-[9px] mt-1 font-normal px-1 py-0 h-4">
                      {q.majorName || '通用'}
                    </Badge>
                  </div>
                ))}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Select
              value={rdqDrawMode}
              onValueChange={(v) => {
                const mode = v as 'random' | 'manual'
                setRdqDrawMode(mode)
                updateState({
                  methodResourceConfigs: {
                    ...state.methodResourceConfigs,
                    random_draw: { ...rdCfg, drawMode: mode },
                  },
                })
              }}
            >
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                setRdqDrawCount(count)
                updateState({
                  methodResourceConfigs: {
                    ...state.methodResourceConfigs,
                    random_draw: { ...rdCfg, drawCount: count },
                  },
                })
              }}
            ...
            <Textarea
              ...
              onChange={(e) => {
                ...
                updateState({
                  methodResourceConfigs: {
                    ...state.methodResourceConfigs,
                    random_draw: { ...rdCfg, submitFormatDesc: v },
                  },
                })
              }}
            ...
            <Textarea
              ...
              onChange={(e) => {
                ...
                updateState({
                  methodResourceConfigs: {
                    ...state.methodResourceConfigs,
                    random_draw: { ...rdCfg, venueResources: v },
                  },
                })
              }}
          ...
          <div className="space-y-4 py-4">
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) => setNewRdqForm({ ...newRdqForm, description: e.target.value })}
                placeholder="输入题目描述"
                className="mt-1.5"
                rows={3}
              />
