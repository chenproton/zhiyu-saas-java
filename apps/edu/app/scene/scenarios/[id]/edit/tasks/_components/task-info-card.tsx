...
export function TaskInfoCard({
  ...
  return (
    <div className="space-y-4">
      {showName && <div><Label>任务名称</Label><Input value={name} onChange={e => onNameChange(e.target.value)} className="mt-1.5" /></div>}
      {showType && (
        <div>
          <Label>任务类型</Label>
      ...
      {showBackground && <div><Label>背景介绍</Label><Textarea value={background || ""} onChange={e => onBackgroundChange?.(e.target.value)} className="mt-1.5" rows={3} /></div>}
    </div>
  )
}
