      ...
      <div className="space-y-4">
        <div className="border rounded-xl p-4">
          ...
          <div className="space-y-2">
                ...
                return (
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        setSelectedPaperForDetail(paper.id)
            ...
            {getLoadedExams().filter((p) => !pSearch || p.name.includes(pSearch)).length === 0 &&
              !pSearch && (
                <div className="text-center py-8 text-gray-400">
                  <Info className="h-8 w-8 mx-auto mb-2 opacity-50" />
                  <p className="text-sm">暂无可选试卷</p>
                  <p className="text-xs mt-1">
                    请点击「新建试卷」创建试卷，或在测评中心准备试卷后刷新
                  </p>
                </div>
              )}
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              试卷「{getLoadedExams().find((e) => e.id === configPaperId)?.name || ''}
              」已创建并选中。 你可以在试卷管理页面中配置题目、分数等。
            </DialogDescription>
          </DialogHeader>
