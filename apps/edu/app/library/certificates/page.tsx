...
export default function CertificatesPage() {
  ...
  const confirmDelete = async () => {
    if (!deleteTarget) return; try { await certificateLibraryApi.delete(deleteTarget); toast({ title: "删除成功" }); loadItems() }
  ...
  return (
    <LibraryPageShell
      title="岗位证书库"
      statLabel="证书总数"
      statIcon={<div className="w-10 h-10 rounded-lg bg-rose-500/10 flex items-center justify-center"><Award className="size-5 text-rose-600" /></div>}
