...
export default function KnowledgePointsPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      const res = await knowledgeApi.list(params); setItems(res.items)
  ...
  const loadGranularCourses = async () => {
    try {
      ...
      setGranularCourses(res.items.map((c) => ({ id: c.id, name: c.name, code: c.code, description: c.description || undefined })))
    } catch { setGranularCourses([]) }
  }

  useEffect(() => { let c = false; ;(async () => { await loadItems() })(); return () => { c = true } }, [loadItems])
  useEffect(() => { let c = false; ;(async () => { await loadGranularCourses() })(); return () => { c = true } }, [])

  const handleOpenAdd = () => { setEditingItem(null); setDialogMode("add"); setLinked(false); setIsDialogOpen(true) }
  const handleOpenEdit = (item: KnowledgePoint) => { setEditingItem(item); setDialogMode("edit"); setLinked(item.linked); setIsDialogOpen(true) }
  ...
  const handleSave = async (values: KnowledgePointFormValues) => {
    try {
      if (editingItem) {
        await knowledgeApi.update(editingItem.id, { name: values.name, code: values.code || undefined, description: values.description || undefined, linked, granularLessonIds: values.granularLessonIds } as any)
        toast({ title: "更新成功" })
      } else {
        await knowledgeApi.create({ name: values.name, code: values.code || undefined, description: values.description || undefined, linked, granularLessonIds: values.granularLessonIds } as any)
        toast({ title: "创建成功" })
      }
      setIsDialogOpen(false); loadItems(); loadGranularCourses()
  ...
  const handleCreateGranularLesson = async (): Promise<string | undefined> => {
    ...
    try {
      const created = await courseApi.create({ name: `基于「${baseName}」的颗粒课`, type: "granular", category: "专业基础" } as any)
      setGranularCourses((prev) => [...prev, { id: created.id, name: created.name, code: created.code, description: created.description || undefined }])
      if (editingItem) {
        await knowledgeApi.update(editingItem.id, { name: editingItem.name, code: editingItem.code || undefined, description: editingItem.description || undefined, linked: editingItem.linked, granularLessonIds: [...(editingItem.granularLessonIds || []), created.id] } as any)
        loadItems()
      }
      setNavigateCourseId(created.id)
      return created.id
    } catch (err: any) { toast({ variant: "destructive", title: "创建颗粒课失败", description: err.message }); return undefined }
  }
  ...
  return (
    <>
      <LibraryPageShell
        title="知识点管理"
        statLabel="知识点总数"
        statIcon={<div className="w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center"><BookOpen className="size-5 text-blue-600" /></div>}
      ...
      <ConfirmDialog
        ...
        confirmText="前往"
        onConfirm={() => {
          if (navigateCourseId) { window.open(`/lesson/admin/granular/add?id=${navigateCourseId}`, "_blank"); setNavigateCourseId(null) }
        }}
      />
      <KnowledgePointFormDialog
        open={isDialogOpen}
        ...
        mode={dialogMode}
        initialValues={editingItem ? { name: editingItem.name, description: editingItem.description || "", code: editingItem.code || "", granularLessonIds: editingItem.granularLessonIds || [] } : undefined}
