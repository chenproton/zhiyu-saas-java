...
import {
  ...
  Monitor, Ellipsis, Eye, Search, Sparkles,
  RotateCcw, Flame, Filter, Archive,
  Calendar, Building2, Tag, Clock, X, ExternalLink, GraduationCap,
} from "lucide-react"
...
export default function LibraryLandingPage() {
  ...
  const orgNames = useMemo(() => {
    ...
    for (const r of resources) { if (r.uploaderOrgName) set.add(r.uploaderOrgName) }
  ...
  const majorNames = useMemo(() => {
    const set = new Set<string>()
    if (orgFilter === "全部") {
      for (const r of resources) { if (r.uploaderMajorName) set.add(r.uploaderMajorName) }
    } else {
      for (const r of resources) { if (r.uploaderOrgName === orgFilter && r.uploaderMajorName) set.add(r.uploaderMajorName) }
    }
    return Array.from(set).sort()
  }, [resources, orgFilter])
  ...
  const hasFilters = timeFilter !== "all" || search || typeFilter !== "全部" || orgFilter !== "全部" || majorFilter !== "全部"
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "40px 20px 0", background: "#f7f8fc" }}>
              ...
              return (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 8 }}>
                    <span style={{ color: TYPE_COLORS[type] || TYPE_COLORS.other }}>{TYPE_EMOJI[type] || "📦"}</span>
                  </div>
                  <div style={{ fontSize: 26, fontWeight: 800, color: TYPE_COLORS[type] || TYPE_COLORS.other, lineHeight: 1, textAlign: "center" as const }}>{count}</div>
                  <div style={{ fontSize: 11, fontWeight: 500, color: "#64748b", marginTop: 4, textAlign: "center" as const }}>{RESOURCE_TYPE_LABELS[type] || "其他"}</div>
                </button>
              )
            })}
          </div>
        ...
        <section style={{ marginBottom: 50 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", padding: 20 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {TIME_RANGES.map(range => (
                  <span key={range.value} onClick={() => setTimeFilter(range.value)}
                    style={{ padding: "4px 14px", borderRadius: 4, cursor: "pointer", color: timeFilter === range.value ? "#2563eb" : "#64748b", background: timeFilter === range.value ? "#eff6ff" : "transparent", fontWeight: timeFilter === range.value ? 500 : 400, transition: "all 0.3s" }}>
                    {range.label}
                  </span>
                ))}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", padding: "8px 0", borderBottom: "1px dashed #f1f5f9", fontSize: 13 }}>
              <span style={{ color: "#94a3b8", width: 80, flexShrink: 0, display: "flex", alignItems: "center", gap: 6 }}>
                <Building2 style={{ width: 14, height: 14 }} />院系筛选：
              </span>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {["全部", ...orgNames].map(name => (
                  <span key={name} onClick={() => { setOrgFilter(name); setMajorFilter("全部") }}
                    style={{ padding: "4px 14px", borderRadius: 4, cursor: "pointer", color: orgFilter === name ? "#2563eb" : "#64748b", background: orgFilter === name ? "#eff6ff" : "transparent", fontWeight: orgFilter === name ? 500 : 400, transition: "all 0.3s" }}>
                    {name}
                  </span>
                ))}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", padding: "8px 0", fontSize: 13 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {["全部", ...majorNames].map(name => (
                  <span key={name} onClick={() => orgFilter === "全部" ? null : setMajorFilter(name)}
                    style={{ padding: "4px 14px", borderRadius: 4, cursor: orgFilter === "全部" ? "not-allowed" : "pointer", color: majorFilter === name ? "#2563eb" : "#64748b", background: majorFilter === name ? "#eff6ff" : "transparent", fontWeight: majorFilter === name ? 500 : 400, opacity: orgFilter === "全部" ? 0.4 : 1, transition: "all 0.3s" }}>
                    {name}
                  </span>
                ))}
            ...
            {hasFilters && (
              <div style={{ marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button onClick={() => { setSearch(""); setTypeFilter("全部"); setTimeFilter("all"); setOrgFilter("全部"); setMajorFilter("全部") }}
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: `1px solid ${color}20`, textAlign: "left" as const,
                      width: "100%", display: "flex", flexDirection: "column",
                      opacity: hasPreview ? 1 : 0.85,
                    }}
                    onMouseEnter={(e) => {
                      if (hasPreview) { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = `0 14px 28px ${color}18`; e.currentTarget.style.borderColor = color }
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 1px 3px rgba(0,0,0,0.06)"; e.currentTarget.style.borderColor = `${color}20`
                    }}
                  >
                    ...
                    <div style={{ padding: 18 }}>
                      <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 12 }}>
                        <div style={{
                          ...
                          fontSize: 24, flexShrink: 0, boxShadow: `0 4px 12px ${color}20`,
                        }}>
                      ...
                      {(resource.uploaderOrgName || resource.uploaderMajorName) && (
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6, fontSize: 11, color: "#94a3b8" }}>
                          {resource.uploaderOrgName && <span style={{ display: "flex", alignItems: "center", gap: 3 }}><Building2 style={{ width: 11, height: 11 }} />{resource.uploaderOrgName}</span>}
                          {resource.uploaderMajorName && <span style={{ display: "flex", alignItems: "center", gap: 3 }}><GraduationCap style={{ width: 11, height: 11 }} />{resource.uploaderMajorName}</span>}
                        </div>
                      )}

                      {resource.description && (
                        <p style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.6, marginBottom: 10, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as any }}>
                          {resource.description}
                        </p>
                      )}

                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "auto", paddingTop: 10, borderTop: "1px solid #f8fafc", fontSize: 11, color: "#cbd5e1" }}>
                        <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <Clock style={{ width: 12, height: 12 }} />
                          {new Date(resource.createdAt).toLocaleDateString("zh-CN", { month: "numeric", day: "numeric" })}
                        </span>
                        {resource.fileSize != null && <span>{formatSize(resource.fileSize)}</span>}
                        {hasPreview && <span style={{ color, fontWeight: 500, display: "flex", alignItems: "center", gap: 3 }}>预览 <Eye style={{ width: 12, height: 12 }} /></span>}
                      </div>
                    </div>
                  </button>
                )
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent style={{ maxWidth: "90vw", maxHeight: "90vh", height: "90vh", padding: 0, overflow: "hidden", borderRadius: 16 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: "1px solid #e2e8f0", background: "#f8fafc" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ fontSize: 20, width: 36, height: 36, borderRadius: 10, background: TYPE_GRADIENTS[previewResource?.resourceType || "other"] || "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {TYPE_EMOJI[previewResource?.resourceType || "other"] || "📦"}
              </div>
              <div>
                <div style={{ fontSize: 14, fontWeight: 600, color: "#1e293b" }}>{previewResource?.name}</div>
                <div style={{ fontSize: 12, color: "#94a3b8" }}>{previewResource && RESOURCE_TYPE_LABELS[previewResource.resourceType]}</div>
              </div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              {previewResource?.url && (
                <a href={previewResource.url} target="_blank" rel="noreferrer"
                  style={{ fontSize: 12, color: "#2563eb", textDecoration: "none", fontWeight: 500, display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: 8, background: "#eff6ff" }}>
                  <ExternalLink style={{ width: 14, height: 14 }} />新窗口打开
                </a>
              )}
          ...
          {kkFileViewUrl && (
            <iframe src={kkFileViewUrl} style={{ width: "100%", height: "calc(100% - 64px)", border: "none" }} title="资源预览" />
          )}
        </DialogContent>
      </Dialog>
      ...
      <footer style={{background: '#141a2e', width: '100%'}}>
        ...
        <div style={{padding: '48px 5% 32px'}}>
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 32}}>
            <div><h3 style={{fontSize: 15, fontWeight: 600, color: '#fff', margin: '0 0 12px 0'}}>场景化数智教学服务平台</h3><p style={{fontSize: 13, color: '#a8b3cf', lineHeight: 1.8, margin: 0}}>专注职业教育数字化</p><div style={{fontSize: 12, color: '#6b7a99', marginTop: 8}}>版本：V3.2.1</div></div>
            <div><h3 style={{fontSize: 15, fontWeight: 600, color: '#fff', margin: '0 0 12px 0'}}>教学资源</h3><p style={{fontSize: 13, color: '#a8b3cf', lineHeight: 1.8, margin: 0}}>岗位标准、实践场景、企业导师</p></div>
            <div><h3 style={{fontSize: 15, fontWeight: 600, color: '#fff', margin: '0 0 12px 0'}}>技术支持</h3><ul style={{listStyle: 'none', padding: 0, margin: 0}}><li style={{fontSize: 13, color: '#a8b3cf', lineHeight: 1.8}}>服务热线：400-888-8888</li><li style={{fontSize: 13, color: '#a8b3cf', lineHeight: 1.8}}>邮箱：support@example.com</li></ul></div>
            <div><h3 style={{fontSize: 15, fontWeight: 600, color: '#fff', margin: '0 0 12px 0'}}>校内支持</h3><ul style={{listStyle: 'none', padding: 0, margin: 0}}><li style={{fontSize: 13, color: '#a8b3cf', lineHeight: 1.8}}>授权院校：XX职业技术学院</li><li style={{fontSize: 13, color: '#a8b3cf', lineHeight: 1.8}}>校内管理员：张老师</li></ul></div>
          </div>
          ...
          <div style={{display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, fontSize: 12, color: '#6b7a99'}}>
            <div><a href="#" style={{color: '#6b7a99', textDecoration: 'none'}}>隐私政策</a><span style={{color: '#29324a'}}>&nbsp;|&nbsp;</span><a href="#" style={{color: '#6b7a99', textDecoration: 'none'}}>用户协议</a></div>
