  ...
  return (
    <div className="space-y-6">
                ...
                {responsibilities.map((resp, i) => {
                  ...
                  return (
                    <tr key={resp.id} className="border-b border-[#f5f5f4] hover:bg-[#fafafa]">
                      ...
                      <td className="p-4">
                        <span className="text-sm font-medium text-purple-600">{items.length} 个</span>
                        <Button
                          variant="outline"
                          size="sm"
      ...
      <div>
        <h4 className="text-base font-medium text-[#1f2937] mb-4 flex items-center gap-2">
          <ListChecks className="w-5 h-5 text-purple-600" />
          任职要求(<strong className="text-purple-600 mx-1">{requirements.length}</strong>项)
        </h4>
                ...
                <>
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-6">
                      <button
                        className="w-8 h-8 rounded-full border border-[#e2e8f0] bg-white text-[#64748b] flex items-center justify-center disabled:opacity-40 hover:border-purple-400 hover:text-purple-600"
                        disabled={page <= 0}
                      ...
                      {Array.from({ length: totalPages }, (_, i) => (
                        <button
                          key={i}
                          onClick={() => setPage(i)}
                          className={`w-2 h-2 rounded-full transition-all ${page === i ? 'bg-purple-500 w-5' : 'bg-[#e2e8f0]'}`}
                        />
                      ))}
                      <button
                        className="w-8 h-8 rounded-full border border-[#e2e8f0] bg-white text-[#64748b] flex items-center justify-center disabled:opacity-40 hover:border-purple-400 hover:text-purple-600"
