...
function TreeNodeRow({ node, level, orgTypeMap, typeNameMap, selectableTypes, selectedIds, onToggleSelect, onBatchSelect, collapsedIds, onToggle, visibleIds, searching }: TreeNodeRowProps) {
  ...
  const allChildIds = useMemo(() => hasChildren ? collectLeafIds(node, selectableTypes, typeNameMap) : [], [node, selectableTypes, typeNameMap, hasChildren])
  ...
  return (
    <div>
      <div className={cn("flex items-center gap-1.5 py-1 px-2 text-sm rounded-md transition-colors", level === 0 && isSelectable && "hover:bg-muted")}
      ...
      {hasChildren && expanded && children.map((child: any) => (
        <TreeNodeRow key={child.id} node={child} level={searching ? 0 : level + 1} orgTypeMap={orgTypeMap}
          typeNameMap={typeNameMap} selectableTypes={selectableTypes} selectedIds={selectedIds} onToggleSelect={onToggleSelect}
          onBatchSelect={onBatchSelect} collapsedIds={collapsedIds} onToggle={onToggle}
          visibleIds={visibleIds} searching={searching} />
      ))}
    </div>
  )
}
...
export function MultiOrgNodePicker({
  ...
  const displayText = value.length === 0 ? placeholder : value.length <= 3 ? value.map((id) => getNodeName(id)).join("、") : `已选 ${value.length} 个班级`
            ...
            {orgLoading ? (
              ...
              orgTree.map((node) => (
                <TreeNodeRow key={node.id} node={node} level={0} orgTypeMap={orgTypeMap}
                  ...
                  collapsedIds={collapsedIds} onToggle={handleToggle}
                  visibleIds={visibleIds} searching={!!search} />
              ))
            )}
          </ScrollArea>
          <DialogFooter>
