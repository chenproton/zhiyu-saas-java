...
const isBank = (type: "bank" | "exam") => type === "bank"

function confirmConfig(type: "bank" | "exam"): Record<Exclude<ConfirmType, null>, { title: string; description: string; variant: 'default' | 'destructive' }> {
  const b = isBank(type)
  return {
    delete: {
      title: "确认删除",
      description: b ? "删除后将无法恢复，题库中的所有题目也会被删除。确定要删除吗？" : "删除后将无法恢复。确定要删除这份试卷吗？",
      variant: "destructive",
    },
    submit: {
      title: "提交审批",
      description: b ? "提交后题库将进入审批流程，审批期间无法编辑。确定要提交吗？" : "提交后试卷将进入审批流程，试卷内容将生成快照。确定要提交吗？",
      variant: "default",
    },
    withdraw: {
      title: "撤回审批",
      description: b ? "撤回后题库将回到未提交状态，可以继续编辑。确定要撤回吗？" : "撤回后试卷将回到未提交状态，可以继续编辑。确定要撤回吗？",
      variant: "default",
    },
    approve: {
      title: "审批通过",
      description: b ? "通过后题库将进入已通过状态。确定要通过吗？" : "通过后试卷将进入已通过状态，可执行发布。确定要通过吗？",
      variant: "default",
    },
    reject: {
      title: "驳回审批",
      description: b ? "驳回后题库将回到已驳回状态，需要修改后重新提交。确定要驳回吗？" : "驳回后试卷将回到已驳回状态，需要修改后重新提交。确定要驳回吗？",
      variant: "destructive",
    },
    publish: {
      title: b ? "发布题库" : "发布试卷",
      description: b ? "发布后题库将对外可见，且无法再编辑。确定要发布吗？" : "发布后试卷将对外可见，且无法再编辑。确定要发布吗？",
      variant: "default",
    },
    unpublish: {
      title: "取消发布",
...
export function EvaluationStatusActions({
  ...
  const handleConfirm = () => {
    ...
    switch (confirmType) {
      ...
      case 'publish':
        ...
        break
      case 'unpublish':
        onStatusChange('unpublish' as StatusAction)
        break
    }
    setConfirmType(null)
  }
  ...
  const canUnpublish = !b && canPerformAction(status, 'unpublish')
    ...
    <>
      ...
      {confirmType && (
        <ConfirmDialog
          ...
          title={cfg[confirmType].title}
          description={cfg[confirmType].description}
          variant={cfg[confirmType].variant}
