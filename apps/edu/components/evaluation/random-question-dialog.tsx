...
export function RandomQuestionDialog({
  ...
  const allocateProportions = useCallback(
    (total: number, entries: string[], weights: Record<string, number>): Record<string, number> => {
                ...
                return (
                  <div key={q.id} className="flex items-center gap-3 rounded-lg border p-3">
                    ...
                    <Button
                      ...
                      onClick={() => removeFromPreview(q.id)}
                    >
                      <X className="size-4" />
                    </Button>
                  </div>
          ...
          <div className="rounded-lg border p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <Input
                ...
                value={count}
                onChange={(e) =>
                  setCount(Math.max(1, Math.min(countSliderMax, parseInt(e.target.value) || 1)))
                }
