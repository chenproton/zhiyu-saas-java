...
export function QuestionFormDialog({
  ...
  const answerToIndexes = useCallback(
    (ans: string | string[], opts: string[]): string | string[] => {
  ...
  const isMultipleAnswerChecked = useCallback(
    (index: number) => {
  ...
  const getKnowledgePointName = useCallback(
    (id: string) => {
  ...
  const CardTitle = ({
    ...
    <div className="mb-3 flex items-center gap-2">
      {Icon && (
        <div
          className={`flex size-6 items-center justify-center rounded-md ${color} text-white shadow-sm`}
        >
              ...
              {blankCount === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-2">
                  {Array.from({ length: blankCount }).map((_, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 rounded-lg border border-border/60 bg-muted/20 px-2 py-1.5 transition-colors focus-within:border-primary/40 focus-within:bg-background focus-within:ring-1 focus-within:ring-primary/10"
                    >
                      <Badge
                        variant="default"
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {knowledgePointIds.map((id) => (
                    <Badge
                      key={id}
                      variant="secondary"
                      className="gap-0.5 rounded-md bg-primary/10 px-1.5 py-0 text-[11px] font-medium text-primary hover:bg-primary/15"
                    >
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-b bg-gradient-to-r from-background via-background to-muted/30 px-6 py-4">
          <DialogHeader className="m-0 gap-1.5 text-left">
            <div className="flex items-center gap-2.5">
              <div
                className={`flex size-9 items-center justify-center rounded-xl ${TYPE_COLORS[type]} text-white shadow-md`}
              >
                {TYPE_ICONS[type]}
              </div>
              <div>
        ...
        <div className="flex flex-1 overflow-hidden bg-gradient-to-br from-muted/20 via-background to-muted/10">
          <div className="flex w-[55%] flex-col gap-4 overflow-y-auto p-5">{renderLeftPane()}</div>
          <div className="my-5 w-px bg-gradient-to-b from-transparent via-border to-transparent" />
          <div className="flex w-[45%] flex-col gap-4 overflow-y-auto p-5">{renderRightPane()}</div>
        </div>
      </DialogContent>
    </Dialog>
  )
