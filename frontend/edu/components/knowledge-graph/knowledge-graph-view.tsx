...
import { Briefcase, FileWarning, Target, BookOpen, Lightbulb, X } from 'lucide-react'
import {
  ReactFlow,
  Controls,
  Background,
...
function KnowledgeGraphNode({ data, selected }: NodeProps<Node<KGNodeData, 'knowledgeGraph'>>) {
  ...
  return (
    <div
      className={cn('flex flex-col items-center transition-opacity', data.dimmed && 'opacity-20')}
    >
      ...
      <div
        className={cn(
          ...
          data.highlight
            ? 'animate-pulse ring-4 ring-red-400/70 ring-offset-2 ring-offset-background shadow-lg shadow-red-500/40'
            : '',
        )}
        style={{
      ...
      items.forEach((n, i) => {
        ...
        result.push({
          ...
          data: {
            ...
            type: n.type,
            dimmed,
            highlight: highlightNodeIds?.has(n.id) ?? false,
          },
        ...
        >
          ...
          >
            ...
            <Controls
              ...
              className="!rounded-lg !shadow !border !bg-background"
            />
          </ReactFlow>
