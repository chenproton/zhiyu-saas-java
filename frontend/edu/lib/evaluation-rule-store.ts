...
export type ScoreTypeField =
  'randomDrawScoreType' | 'reviewScoreType' | 'outcomeScoreType' | 'homeworkScoreType'

export type RubricIdField =
  'randomDrawRubricId' | 'reviewRubricId' | 'outcomeRubricId' | 'homeworkRubricId'

export type QuestionIdField = 'randomDrawQuestions' | 'questionBankQuestions' | 'quizQuestions'
...
export function reducer(state: EvalRuleState, action: EvalRuleAction): EvalRuleState {
  ...
  switch (action.type) {
    ...
    case 'REMOVE_EVAL_POINT': {
      const field = action.field
      ;(next as any)[field] = ((next as any)[field] as EvalRulePoint[]).filter(
        (p) => p.id !== action.id,
      )
      return next
    }
...
export function useEvalRuleStore(options: UseEvalRuleStoreOptions) {
  ...
  const initialState = useMemo<EvalRuleState>(() => {
    ...
    return initialConfig
      ? { ...base, ...initialConfig, evaluationMethods: normalizedMethods }
      : base
  ...
  const setMethodEvalSubjects = useCallback(
    (methodKey: string, subjects: EvalRuleSubjectConfig[]) => {
  ...
  const updateEvalPoint = useCallback(
    (field: EvalPointField, id: string, updates: Partial<EvalRulePoint>) => {
  ...
  const setResourceConfig = useCallback(
    (methodKey: string, resourceConfig: Record<string, any>) => {
      dispatch({ type: 'SET_RESOURCE_CONFIG', methodKey, resourceConfig })
    },
    [],
  )

  const setCustomQuestions = useCallback(
    (questions: EvalRuleConfig['randomDrawCustomQuestions']) => {
