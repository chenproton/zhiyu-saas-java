    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span class="card-title">岗位批次</span>
      ...
      <el-table v-loading="loading" :data="items" stripe>
        ...
        <el-table-column prop="pendingCount" label="待审" width="90" />
...
import { batchApi } from '@/api/job';
import type { JobBatch } from '@/types/job';

const items = ref<JobBatch[]>([]);
const loading = ref(false);
const dialog = ref(false);
const saving = ref(false);
const editing = ref<JobBatch | null>(null);
const form = reactive({ name: '', code: '' });

async function loadItems() {
  loading.value = true;
  try {
    const res = await batchApi.list({ limit: 200 });
...
function openDialog(row?: JobBatch) {
...
async function save() {
  ...
  try {
    ...
    if (editing.value) { await batchApi.update(editing.value.id, payload); ElMessage.success('更新成功'); }
    else { await batchApi.create(payload); ElMessage.success('创建成功'); }
    dialog.value = false;
    loadItems();
  } catch (e) { ElMessage.error((e as Error).message || '保存失败'); } finally { saving.value = false; }
}
async function toggleStatus(row: JobBatch) {
  try {
    await batchApi.updateStatus(row.id, row.status === 'open' ? 'closed' : 'open');
    ElMessage.success('状态已更新');
    loadItems();
  } catch (e) { ElMessage.error((e as Error).message || '操作失败'); }
}
async function confirmDelete(row: JobBatch) {
  try { await ElMessageBox.confirm('确定要删除该批次吗？', '确认删除', { type: 'warning', confirmButtonText: '删除', cancelButtonText: '取消' }); } catch { return; }
  try { await batchApi.delete(row.id); ElMessage.success('删除成功'); loadItems(); }
