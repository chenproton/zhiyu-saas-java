        ...
        <template v-else-if="detailData">
          <template v-if="detail?.kind === 'project'">
            <div class="detail-grid">
              ...
              <div><div class="l">当前阶段</div><div class="v">{{ (detailData as any).phase || '-' }}</div></div>
              <div><div class="l">发布状态</div><div class="v">{{ (detailData as any).publishStatus || '-' }}</div></div>
            ...
            <div v-if="(detailData as any).milestones?.length" class="milestones">
              ...
              <div v-for="m in (detailData as any).milestones" :key="m.id" class="milestone">
                <div class="m-name">{{ m.name }} <el-tag :type="m.isCompleted ? 'success' : 'info'" size="small">{{ m.isCompleted ? '已完成' : '未完成' }}</el-tag></div>
          ...
          <template v-else-if="detail?.kind === 'achievement'">
            <div class="detail-grid">
              <div><div class="l">成果类型</div><div class="v">{{ (detailData as any).type || '-' }}</div></div>
              <div><div class="l">成果状态</div><div class="v">{{ (detailData as any).status || '-' }}</div></div>
              ...
              <div><div class="l">成果归属人</div><div class="v">{{ ((detailData as any).ownerPersons || []).join('、') || '-' }}</div></div>
              <div><div class="l">成果共建人</div><div class="v">{{ ((detailData as any).coBuilders || []).join('、') || '-' }}</div></div>
            </div>
          </template>
          <template v-else>
            <div class="detail-grid">
              <div><div class="l">协议类型</div><div class="v">{{ (detailData as any).type || '-' }}</div></div>
              <div><div class="l">协议状态</div><div class="v">{{ (detailData as any).status || '-' }}</div></div>
              <div><div class="l">开始日期</div><div class="v">{{ fmt((detailData as any).startDate) }}</div></div>
              <div><div class="l">结束日期</div><div class="v">{{ fmt((detailData as any).endDate) }}</div></div>
...
function schoolRows(school: PartnerCooperationSchool): Row[] {
  const items: Row[] = [];
  for (const p of school.projects || []) items.push({ kind: 'project', id: p.id, name: p.name, label: p.phase, updatedAt: p.updatedAt });
  for (const a of school.achievements || []) items.push({ kind: 'achievement', id: a.id, name: a.title, label: a.type, updatedAt: a.updatedAt });
  for (const g of school.agreements || []) items.push({ kind: 'agreement', id: g.id, name: g.name, label: g.status, updatedAt: g.updatedAt });
