...
async function load() {
  ...
  try {
    const [userRes, orgRes, orgTypeRes] = await Promise.all([
      userManagementApi.list({ roleCode: 'student', limit: 1000 }),
    ...
    const classTypeIds = new Set(
      (orgTypeRes.items || []).filter((t) => t.name === '班级').map((t) => t.id)
    );
    // 班级树：仅保留「班级」类型的节点（含其祖先层级用于展示）
    const byId = new Map<string, any>();
    const buildTree = (nodes: any[]): any[] => nodes.map((n) => {
      const node = { value: n.id, label: n.name, children: n.children ? buildTree(n.children) : [] };
      byId.set(n.id, n);
      return node;
    });
    const fullTree = buildTree(orgs);
    // 只保留班级叶子（树形展示时过滤掉非班级中间层）
    ...
    students.value = allUsers.value.map((u) => {
      ...
      if (classNode) {
        ...
        if (department === '—') department = classNode.parentId ? (byId.get(classNode.parentId)?.name || '—') : '—';
      }
      return {
        id: u.id,
        name: u.name,
