...
- **API 对象分组**：7 大分组 40+ API 对象，覆盖核心市场、后台管理、Portal 管理、岗位/场景/课程/评价等平台后端接口。超管控制台 `/api/v1/admin/tenants` 系列接口暂未在前端封装。
...
| API 覆盖 | PASS | 40+ API 对象，覆盖除 `/admin/tenants` 超管接口外的全部后端接口；`userManagementApi.batchCreate` 与 `roleApi.assign` 请求体已与后端对齐 |
...
- 超管控制台接口暂未在 `api-client` 中封装，若前端需要跨租户管理功能需补充对应 API。
