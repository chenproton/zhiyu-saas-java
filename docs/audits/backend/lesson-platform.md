...
- **课程资源绑定**：`CourseResourceHandler` 管理课程级资源（`node_resources` 表），支持按课程 ID 和资源类型筛选、创建/更新/删除课程资源。- **课程节点树**：`system_course_nodes` 通过 `parent_id` 实现层级树。节点关联知识点通过 `node_knowledge_point_bindings` 表，节点关联资源通过 `CourseResourceHandler`（`course_resource_handler.go`）管理。
...
| 内容状态机 | PASS | 完整状态流转 |
| 课程节点树 | PASS | CRUD + 排序；CourseResourceHandler 管理节点资源绑定；知识点通过绑定表关联 |
...
| 课程资源管理 | PASS | `CourseResourceHandler` 管理课程级资源；按课程 ID 和类型筛选；创建/更新/删除 |
