...
现有「AI 智能服务平台」（`ai` 模块）只有通用 AI 助手对话与 AI 辅助表单填写，缺少知识沉淀与智能体搭建能力。旧系统 zhiyu-ai（Python/FastAPI 单体）验证过「知识库 + 智能体 + 广场 + 审核上架」的产品形态，现按本仓库 Go + React SPA 技术栈与多租户 SaaS 架构重写。
...
- **v2.2 A+B 优化包**（用户选定）：A1 YIKnow 会话持久化（172 迁移 agent_id 可空=通用会话；POST /ai/yiknow/chat SSE 落库、GET /ai/yiknow/conversations 列表、复用 /ai/conversations/{id} 读写删）；A2 知识库详情页贡献者区+文档表格化（名称/类型/贡献者/上传时间/状态）；A3 大厅搜索/筛选/排序/页码 URL 化 + LandingPagination 页码分页（取代加载更多）；A4 广场/大厅空态引导 CTA；B5 浏览量（卡片 👁 展示，sort=views 进排序白名单）；B6 知识库问答记录（171 ai_kb_asks，ask 落库 + GET /ai/kb/{id}/asks 我的历史并入问答区）；B7 编辑器实时试聊（POST /ai/agents/{id}/preview，owner 专属、用表单当前提示词、不落库不计数）；B8 统一收藏列表纳入 ai_kb/ai_agent（GET /favorites 响应扩展，收藏页 AI 服务分组）。测试环境治理：testhelper 自动补种辅助用户/租户（RequireActiveUser 适配），修复 8 个历史腐坏测试，后端全套件转绿。
- **v2.8 对话全弹窗化 + 侧栏收敛**（用户拍板）：独立对话页 `/portal/apps/ai/chat` **下线**（路由删除），AI 对话统一走 YIKnowChatDialog 弹窗（应用中心「YI Know 助手」卡片、落地页「立即体验」、右下角浮动机器人三入口共用，弹窗逻辑不变）；AI 平台侧边栏移除「YI Know 助手」导航项（仅剩「AI 广场管理」组，库详情/智能体详情/创作页高亮归该组），`brandHref` 改指落地页；应用中心「YI Know 助手」卡片保留为弹窗入口（href 指向已下线 chat 路由仅作弹窗标识，不跳转）。YIKnowChat page 变体保留（未来页面化复用）。
- **v2.7.4 机器人显隐联动**：任意入口（卡片/落地页/机器人自身）打开 YIKnow 弹窗时右下角机器人整体隐藏（模块级打开计数广播）；机器人悬停白色关闭按钮（demo 残留）移除。
- **v2.7.2 弹窗统一 + v2.7.3 对话体验打磨**：全站唯一 YIKnowChatDialog（居中+遮罩+毛玻璃/渐变线/深阴影，三入口共用）；体验对齐最佳实践——智能跟随滚动（上翻不拽回）、停止生成键、气泡 hover 复制+末条重新生成、Textarea 自适应（Enter/Shift+Enter）、会话重命名（双击/hover 铅笔，PATCH /ai/conversations/{id}，空标题 400/他人 404）、历史按日期分组（今天/昨天/7天内/更早）、流式失败气泡内「点击重试」、代码块一键复制、移动端左轨抽屉化。
- **v2.7.1 浮动机器人接入真实对话 + 我的资产**：① 全局右下角 YI KNOW 机器人（app/portal/layout 挂载的 yi-know-assistant）面板内容由 demo 替换为真实 YIKnowChat（modal 变体，960px 底部弹出）；demo 资源浏览/假对话逻辑移除；② YIKnowChat 左侧功能轨「知识库」占位升级为真实「我的知识库」并新增「我的智能体」入口：主区切换为资产列表视图（我创建的=listMine owned + 我收藏的=favoriteApi.list 的 ai_kb/ai_agent，上下两段），点击条目跳详情/对话页；浮动面板场景经 onNavigate 自动收起。
- **v2.7 聊天弹窗化 + 审核管理闭环**：① YIKnow 聊天体验抽为共享组件（variant page/modal），前台入口（/portal/apps 卡片+常用服务、落地页「立即体验」）统一弹窗开聊（消除按来源的返回导航歧义），/chat 路由保留直达（**v2.8 起下线，对话全弹窗**）；② 审核管理只读体验放行：school_admin 对本租户任意状态 KB/智能体可读详情/文档/提问/对话/预览（resolveKBRole 返回 viewer、GetAgent/AgentChat/PreviewAgent 加 isAdmin），写路径仍按资源角色拒绝；③ 审核工作台每行加「前往使用」（KB→详情页、智能体→对话页）；④ AI 广场管理下新增「知识库管理」「智能体管理」页（/admin/kbs、/admin/agents，复用 reviews 接口三态筛选+分页，操作=前往使用+下架二次确认），权限树同步。
- **v2.6.2 菜单文案重命名**：「AI 助手→YI Know 助手」「平台管理→AI 广场管理」「内容审核→知识库/智能体审核」「第三方挂接→外部 AI 服务上架」（侧栏、/portal/apps 卡片、角色权限树同源同步）；/portal/apps AI 区两张卡片备注补齐（ai-chat 更新、ai-admin 新增）。
- **v2.6.1 KB 编辑页一体化**（用户拍板）：① 新建知识库去弹窗，改为独立页 /portal/apps/ai/studio/kb/new（与 agents/new 同骨架），创建成功跳管理页续传文档；② KB 管理页去 Tabs：顶部一张可折叠大卡（默认展开，对齐 /lesson/admin/system/add）左栏基本信息表单、右栏协作者（邀请+列表），下方整卡文档管理；③ 智能体新建页双栏化（对齐 zhiyu-ai 智能体开发页）：左全量表单，右 sticky 预览对话——保存前为占位卡（预览端点需已有 agent），保存创建后自动进入编辑形态试聊激活（router.replace 到编辑页）。
- **v2.6 创作页改版**（对齐 zhiyu-ai builder 模式，用户拍板「新建/编辑不应跳后台」）：① studio 编辑器（知识库管理/智能体新建+编辑）加入 FULL_WIDTH_PAGES 全宽直出，不再包平台侧边栏；② 共享骨架 `_components/studio-editor-shell.tsx`（sticky 毛玻璃顶栏：返回工坊/图标/标题/状态徽标/操作区 + EditorCard 白色圆角面板）；③ 智能体编辑页改双栏 builder——左表单分三卡片（基本信息/对话设定/关联知识库），右栏 sticky 实时试聊（表单名称/头像/提示词经 onLiveChange 实时镜像到右栏，预览不落库）；④ 知识库管理页 Tabs 卡片化 + 标签计数。
- **v2.5 大厅筛选 + 页面对齐**（用户拍板）：① 大厅页头对齐 /evaluation/landing/exam-center 模式（同宽容器左对齐，去内层 max-w-6xl）；② 去掉大厅统计条；③ 大厅分类筛选（URL 同步，复用 LandingFilterRow）：智能体=所属专业/所属院系/更新时间（7d/30d/180d），知识库另加类型（course_resource 课程资源库/research 科研成果库/teaching_case 教学案例库/qa 问答知识库）；④ 分类字段落库（174：两表加 major_id/department_id，知识库加 kb_type；可空=不限，字典=majors 专业 + organizations 二级学院），新建/编辑表单加配置字段（共享组件 classify-selects.tsx）；⑤ 顶部导航「应用服务中心」仅在 /portal/apps 主页高亮（平台页不再误高亮，对齐 /job/landing）；⑥ hero 第二行改「培养适应复合型岗位新需求人才」，主按钮收敛为单一「逛逛 AI 广场」滚动到广场区块。接口：GET /ai/square/kbs|agents 增 majorId/departmentId/kbType/updated 参数。
- **v2.4 hero 再调**（用户拍板）：标题改两行（对齐 /job/landing 风格：「AI 赋能场景化数智教学」白色 +「培养宽技能数智化人才，适应复合型岗位新需求」white/80）；主按钮改「制作专属智能体/知识库」锚点滚到 #studio；右侧品牌卡定稿为 YI KNOW 标签 → 职业教育场景化教学智能助理 → 口号 → 多元大模型能力说明 → 立即体验。
- **v2.3 hero 改版**（用户拍板文案）：主标题改为「AI 赋能培养宽技能数智化人才，适应复合型岗位新需求」，副标题改为职教定位长文案（场景化数智教学模式/智能教学辅助/岗位能力评估/个性化学习路径规划）；hero 右侧去对话示例气泡、模拟输入框、待上线模块 chips，改为 YIKnow 品牌展示卡（YIKnow · You Ask I Know · 你问，我懂 + 立即体验 → /chat）。
- **v2.2.1 浏览量全局统一**（用户拍板「不用太严谨，记录数加一即可」）：ai_knowledge_bases/ai_agents 的 view_count 列废弃（173 迁移存量搬入全局 view_counters 后删列），浏览计数改走全局 RecordView（view_logs 明细 + view_counters upsert），口径与岗位/场景/联盟一致——浏览详情即 +1、不排 owner、每次重复浏览都计；列表/详情/收藏查询统一以子查询联查 view_counters。
- **v2.1 工坊类型占比环图**：在紧凑面板内恢复左栏环图（v2.0.1 去掉的环图回归），但口径改为**类型占比**（我的知识库/我的智能体/共享给我的，用户要求），右栏卡片混排不变；环图列 220px 与一行卡片同高，不再显著加高面板。
- **v2.0.1 工坊紧凑化**：去掉状态环图侧栏与分组标题，知识库+智能体+共享库混排单网格（4 列），面板压到一行高度；广场/大厅卡片网格同步收窄为 lg:grid-cols-4（与工坊卡片同宽）。
