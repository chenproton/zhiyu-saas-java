...
- **「已改动」判定口径**：以「用户首次交互前」的表单快照为干净基线，当前值与基线不同才算有未保存内容。弹窗打开后异步回填的初始值不算改动；用户改完又改回原值等于没改，可直接关闭。
- **覆盖控件**：原生 input/textarea/select、contenteditable、Radix 勾选框/开关/单选/下拉触发器（按 `aria-checked` 与选中文案比对）；动态增删行也算改动。
- **不算「内容」的控件**：搜索框（`input[type="search"]`）、cmdk 搜索输入；其他需排除的控件加 `data-unsaved-ignore`。
- **逐弹窗开关**：`unsavedGuard`——`'auto'`（默认，自动检测）/ `true`（强制视为有未保存内容，用于画布类自定义编辑器）/ `false`（关闭守卫）。`false` 适用：纯展示弹窗、弹窗内即时保存的场景、勾选即时同步给父级表单的选择器弹窗（关闭本就不丢内容，如资源选择器「添加课程资源」、共建人选择器）。
- 已自行 `onPointerDownOutside={(e) => e.preventDefault()}`（全屏编辑器禁止点外部关闭）的弹窗行为不变：消费方已阻止的关闭不再二次介入。
