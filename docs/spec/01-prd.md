...
| L-4 | 作为 school_admin，我希望发布就业项目（指定参与企业与面向学生群体），企业针对项目录入岗位，学生在供需大厅浏览并投递，以便打通人才与岗位供需链路 | 项目 publish_status(draft/published) 控制上架，展示状态由起止日期派生；浏览全量可见（任何登录角色）；target_groups 组内 AND/组间 OR 控制学生**投递资格**（空=全校可投，不符 403）；岗位可独立或挂项目（挂项目且 published 才上大厅）；投递落档案快照+求职信、(job_id,student_id) 唯一防重；企业端只读查看投递；学校端可下架岗位 |
