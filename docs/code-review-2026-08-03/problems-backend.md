...
| `handler/on_site_question_library_handler.go:137-139` | GET /library/on-site-questions/{id} | ~~完全无鉴权（未登录可读，含答案）~~ **已修正**：走 crudGet，`crudCheckPermit` 有登录检查；原 `GetOwnership: false` 可跨租户读取（含答案） ✅ 已修复（GetOwnership=true，登录+租户校验） |
...
- **domain 类型问题**：`evaluation.go:210` JobAbilityResult.EvaluatedAt string 与 DB timestamptz 不一致（待办）；`lesson.go:137` LessonBatchStatus 枚举缺 'active'（表 DEFAULT）✅ 已修复（补 LessonBatchStatusActive）；`status.go:13-25` 死常量 ✅ 已修复（删 4 个死常量）；`unified.go:99/107` Phone 与 ContactPhone 并存（API 兼容保留）；`alliance.go:62-75/178-193` 重复类型定义 ❌ 误报（企业协议=单企业、通用协议=多关联，为不同实体）
