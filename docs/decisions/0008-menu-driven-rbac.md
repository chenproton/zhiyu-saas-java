...
- **联盟写授权面**：仅勾联盟前台落地页（`/portal/alliance/landing`）的角色是前台只读角色，不授予联盟管理 CRUD（写授权面 = `/portal/apps/alliance` 管理菜单）；
- **权限生效时差**：`RequireMenu` 走「查库 + Redis 60s 缓存」，roles 配置变更最长 60s 生效；`RequireSystemPermission`（关键写白名单）走 JWT claims 登录快照（`SystemMenus`），系统菜单变更需重新登录生效——两套生效机制存在时差，文档化避免误判为 bug。
