...
- **门禁组合**（deploy.sh / CI 强制）：后端 `cd backend/java && ./mvnw compile -q`；前端 `cd frontend/portal-vue && pnpm build` + `cd frontend/plus-ui && pnpm build`；另跑 `./scripts/spec-check.sh`（见 AGENTS.md 4.2）。
