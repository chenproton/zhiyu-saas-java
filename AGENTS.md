...
- 存储：`backend/internal/store/ai_config.go`；加密：`backend/internal/crypto/aes.go`
- 现有端点：`GET/PUT/DELETE /ai/config`、`POST /ai/chat`（`backend/internal/handler/ai_handler.go`）
...
7. 前端调用经 `packages/api-client` 新增方法（`portalRequest` 等）；收到 412 统一引导到 `/portal/apps/system/tenant` 完成配置
