...
python .claude/skills/collaborating-with-gemini/scripts/gemini_bridge.py \
  --cd . \
  --PROMPT "Design an admin dashboard home layout using Element Plus (el-row, el-col, el-card, el-statistic) following the framework's admin UI conventions. Focus on clean, modern design with proper spacing."
```
...
python .claude/skills/collaborating-with-gemini/scripts/gemini_bridge.py \
  --cd "/project" \
  --PROMPT "Design a CRUD list+form page using Element Plus (el-table, el-form, el-dialog) following the framework's codegen conventions (fm/vue templates). OUTPUT: Unified Diff Patch."
```
...
python .claude/skills/collaborating-with-gemini/scripts/gemini_bridge.py \
  --cd "/project" \
  --PROMPT "Review the CSS in the admin login page (Vue + Element Plus) for responsiveness, layout consistency, and Element Plus best practices."
```
