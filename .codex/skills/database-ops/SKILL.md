...
> 真实建表 SQL 见 `scripts/sql/ry_vue.sql`，方言模板见 `ruoyi-modules/ruoyi-gen/src/main/resources/fm/sql/*.sql.ftl`。
...
以下模板对齐 `scripts/sql/ry_vue.sql` 中 `sys_*` 表与代码生成器约定，新业务表照此写：
...
> SQL 文件位置：MySQL 主库 `scripts/sql/ry_vue.sql`、`ry_job.sql`、`ry_workflow.sql`、`ry_ai.sql`；其他库 `scripts/sql/oracle/*.sql`、`scripts/sql/postgres/*.sql`、`scripts/sql/sqlserver/*.sql`。换库部署时跑对应目录脚本。
...
7. **多库优先框架方言封装**：跨 MySQL/Oracle/PG/SQLServer 时用 `DataBaseHelper` / `findInSetIfPresent`，避免硬编码单库函数；换库部署跑 `scripts/sql/{库}/` 对应脚本。
