<!--
  顶部全局导航栏（门户 / 前台 landing 共用，从 PortalLayout 提取）

  对齐来源：原 React 版 `top-nav.tsx`
  （品牌 + 门户首页/我的服务台/应用服务中心 3 项 + 当前时间 + 用户区/登录入口，含 isActive 高亮语义）

  使用方：
  - PortalLayout.vue（管理壳：顶栏 + 左侧域导航 + 内容区）
  - LandingLayout.vue（landing 无壳布局：仅顶栏，对齐 React landing 系列「仅 TopNav」形态）
  两处共用保证全站顶部全局导航栏视觉与行为一致。
