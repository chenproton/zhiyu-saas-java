...
async function load() {
  ...
  try {
    const [userRes, titleRes, orgRes] = await Promise.all([
      userManagementApi.list({ ...(tenantId.value ? { tenantId: tenantId.value } : {}), roleCode: 'teacher', limit: 1000 }),
      staffTitleApi.list({ ...(tenantId.value ? { tenantId: tenantId.value } : {}), limit: 1000 }),
