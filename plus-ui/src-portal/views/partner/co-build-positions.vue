<template>
  <div class="list-page">
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <div>
            <div class="card-title">岗位共建</div>
            <div class="card-sub">为合作学校创建或编辑岗位资源，保存后状态为草稿，由学校端审核发布。</div>
          </div>
    ...
    <el-dialog v-model="createDialog" title="新建岗位" width="440px">
      <el-form label-width="90px">
        ...
        <p class="muted">创建后将生成草稿「未命名岗位」，并跳转到编辑页完善内容。</p>
      </el-form>
      <template #footer>
...
import { partnerCobuildPositionApi, partnerSchoolApi } from '@/api/partner';
import type { CoBuildPosition, PartnerSchool } from '@/types/partner';
...
function goEdit(row: CoBuildPosition) {
  router.push(`/partner/co-build/positions/${row.id}/edit`);
}
...
async function onCreate() {
  ...
  try {
    const created = await partnerCobuildPositionApi.create({
      schoolTenantId: createForm.value.schoolTenantId,
      name: '未命名岗位',
...
async function confirmDelete(row: CoBuildPosition) {
  try {
    await ElMessageBox.confirm(`确定要删除岗位「${row.name}」吗？`, '确认删除', {
      type: 'warning',
  ...
  try {
    await partnerCobuildPositionApi.delete(row.id);
