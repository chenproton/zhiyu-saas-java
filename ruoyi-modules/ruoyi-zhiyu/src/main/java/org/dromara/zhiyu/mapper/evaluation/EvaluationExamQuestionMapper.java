...
public interface EvaluationExamQuestionMapper extends BaseMapperPlus<EvaluationExamQuestion, EvaluationExamQuestion> {
    ...
    @Update("<script>UPDATE exam_questions eq SET score = u.score"
        + " FROM unnest(CAST(#{ids} AS uuid[]), CAST(#{scores} AS float8[])) AS u(question_id, score)"
        + " WHERE eq.exam_id = #{examId}::uuid AND eq.tenant_id = #{tenantId}::uuid AND eq.question_id = u.question_id</script>")
    ...
    @Update("UPDATE exams SET total_score = COALESCE((SELECT SUM(score) FROM exam_questions WHERE exam_id = #{examId}::uuid), 0),"
        + " updated_at = NOW() WHERE id = #{examId}::uuid AND tenant_id = #{tenantId}::uuid")
    int recalcExamTotal(@Param("examId") String examId, @Param("tenantId") String tenantId);

    /** 更新题目分数（限定租户，返回是否命中） */
    @Update("UPDATE exam_questions SET score = #{score} WHERE exam_id = #{examId}::uuid AND question_id = #{questionId}::uuid"
    ...
    @org.apache.ibatis.annotations.Insert("INSERT INTO exam_questions (id, tenant_id, exam_id, question_id, type,"
        ...
        + " #{analysis}, #{score},"
        + " (SELECT COALESCE(MAX(sort_order), 0) + 1 FROM exam_questions WHERE exam_id = #{examId}::uuid))"
    ...
    @org.apache.ibatis.annotations.Delete("<script>DELETE FROM exam_questions"
        + " WHERE exam_id = #{examId}::uuid AND tenant_id = #{tenantId}::uuid"
        + " AND NOT (question_id = ANY(CAST(#{questionIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[])))</script>")
    ...
    @org.apache.ibatis.annotations.Insert("INSERT INTO exam_questions (id, tenant_id, exam_id, question_id, type,"
        ...
        + " ON CONFLICT (exam_id, question_id) DO UPDATE SET type = EXCLUDED.type, content = EXCLUDED.content,"
        + " options = EXCLUDED.options, answer = EXCLUDED.answer, analysis = EXCLUDED.analysis,"
