...
public interface PortalResourceSnapshotMapper extends BaseMapperPlus<PortalResourceSnapshot, PortalResourceSnapshot> {
    ...
    @Select("SELECT JSON_OBJECT("
        ...
        + " 'system_course_nodes', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        ...
        + " ) ORDER BY t.sort_order, t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, tenant_id, course_id, parent_id, name, code,"
        ...
        + " 'node_quizzes', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        + "   'id', t.id, 'node_id', t.node_id, 'title', t.title, 'type', t.type, 'time_limit', t.time_limit"
        + " ) ORDER BY t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, node_id, title, type, time_limit FROM node_quizzes"
        + "   WHERE node_id IN (SELECT id FROM system_course_nodes WHERE course_id = #{courseId} AND tenant_id = #{tenantId})) t),"
        + " 'node_quiz_questions', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        ...
        + " ) ORDER BY t.sort_order, t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, quiz_id, type, question, options, answer, score, sort_order"
        ...
        + " 'hybrid_node_modules', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        + "   'id', t.id, 'node_id', t.node_id, 'module_key', t.module_key, 'mode', t.mode, 'data', t.data"
        + " ) ORDER BY t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, node_id, module_key, mode, data FROM hybrid_node_modules"
        + "   WHERE node_id IN (SELECT id FROM system_course_nodes WHERE course_id = #{courseId} AND tenant_id = #{tenantId})) t)"
        + ")")
     ...
     */
    @Select("SELECT JSON_OBJECT("
        + " 'course_knowledge_bindings', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        + "   'id', t.id, 'course_id', t.course_id, 'knowledge_point_id', t.knowledge_point_id,"
        + "   'bind_type', t.bind_type, 'source_id', t.source_id"
        + " ) ORDER BY t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, course_id, knowledge_point_id, bind_type, source_id"
        + "   FROM course_knowledge_bindings WHERE course_id = #{courseId}) t),"
        + " 'course_resource_bindings', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        + "   'id', t.id, 'course_id', t.course_id, 'resource_id', t.resource_id"
        + " ) ORDER BY t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, course_id, resource_id FROM course_resource_bindings"
        + "   WHERE course_id = #{courseId} AND tenant_id = #{tenantId}) t),"
        + " 'node_knowledge_point_bindings', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        + "   'id', t.id, 'node_id', t.node_id, 'knowledge_point_id', t.knowledge_point_id"
        + " ) ORDER BY t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, node_id, knowledge_point_id FROM node_knowledge_point_bindings"
        + "   WHERE node_id IN (SELECT id FROM system_course_nodes WHERE course_id = #{courseId} AND tenant_id = #{tenantId})) t),"
        + " 'node_resource_bindings', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        + "   'id', t.id, 'node_id', t.node_id, 'resource_id', t.resource_id"
        + " ) ORDER BY t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, node_id, resource_id FROM node_resource_bindings"
        + "   WHERE node_id IN (SELECT id FROM system_course_nodes WHERE course_id = #{courseId} AND tenant_id = #{tenantId})) t),"
        + " 'knowledge_points', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        ...
        + " ) ORDER BY t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, name, code, description, linked, granular_lesson_ids, creator_id,"
        ...
        + "     (SELECT id FROM system_course_nodes WHERE course_id = #{courseId} AND tenant_id = #{tenantId}))) t),"
        + " 'resource_library', (SELECT COALESCE(CAST(CONCAT('[', GROUP_CONCAT(JSON_OBJECT("
        ...
        + " ) ORDER BY t.id SEPARATOR ','), ']') AS JSON), '[]') FROM (SELECT id, tenant_id, name, resource_type, url, description, thumbnail,"
