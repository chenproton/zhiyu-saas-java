...
public interface LessonCourseCloneMapper extends BaseMapperPlus<LessonCourse, LessonCourse> {
    ...
    @Select("SELECT name, type, COALESCE(tenant_id, '') AS tenant_id FROM courses WHERE id = #{id}")
    ...
    @Insert("INSERT INTO courses (id, tenant_id, code, name, type, category, major_id, teacher_id, industry_id, version,"
        ...
        + " online_hours, offline_hours, online_weight, offline_weight, semester, class_name,"
        + " 'draft', cover_color, cover_image, course_tag, difficulty, description, #{creatorId}, co_creator_ids, batch_id,"
    ...
    @Insert("INSERT INTO course_knowledge_bindings (id, tenant_id, course_id, knowledge_point_id, bind_type, source_id)"
        + " SELECT (UUID()), #{tenantId}, #{newId}, knowledge_point_id, bind_type, source_id"
    ...
    @Insert("INSERT INTO course_resource_bindings (id, tenant_id, course_id, resource_id)"
        + " SELECT (UUID()), #{tenantId}, #{newId}, resource_id"
    ...
    @Select("SELECT id, parent_id AS parent_id, name, code, sort_order, ref_type, source_id AS source_id,"
        + " source_name, teaching_goals, detailed_description, description_pdf, background, estimated_hours,"
        + " duration, difficulty, knowledge_point_ids AS knowledge_point_ids,"
        + " resource_ids AS resource_ids, ability_point_ids AS ability_point_ids,"
        + " eval_data AS eval_data, status FROM system_course_nodes WHERE course_id = #{courseId}"
    ...
    @Insert("INSERT INTO node_knowledge_point_bindings (id, node_id, knowledge_point_id)"
        + " SELECT (UUID()), #{newNodeId}, knowledge_point_id FROM node_knowledge_point_bindings WHERE node_id = #{oldNodeId}")
    ...
    @Insert("INSERT INTO node_resource_bindings (id, node_id, resource_id)"
        + " SELECT (UUID()), #{newNodeId}, resource_id FROM node_resource_bindings WHERE node_id = #{oldNodeId}")
    int cloneNodeResourceBindings(@Param("oldNodeId") String oldNodeId, @Param("newNodeId") String newNodeId);

    @Select("<script>SELECT id, node_id AS node_id, title, type, time_limit FROM node_quizzes"
        + " WHERE node_id IN <foreach collection='nodeIds' item='n' open='(' separator=',' close=')'>#{n}</foreach></script>")
    ...
    @Select("<script>SELECT quiz_id AS quiz_id, type, question, options AS options, answer, score, sort_order"
        + " FROM node_quiz_questions WHERE quiz_id IN <foreach collection='quizIds' item='q' open='(' separator=',' close=')'>#{q}</foreach>"
    ...
    @Insert("INSERT INTO node_quiz_questions (id, tenant_id, quiz_id, type, question, options, answer, score, sort_order)"
        + " VALUES (#{id}, #{tenantId}, #{quizId}, #{type}, #{question}, CAST(#{options} AS JSON),"
    ...
    @Select("<script>SELECT node_id AS node_id, module_key, mode, data AS data FROM hybrid_node_modules"
        + " WHERE node_id IN <foreach collection='nodeIds' item='n' open='(' separator=',' close=')'>#{n}</foreach></script>")
