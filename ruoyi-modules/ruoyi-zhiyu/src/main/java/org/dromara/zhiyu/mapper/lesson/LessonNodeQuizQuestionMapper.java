...
public interface LessonNodeQuizQuestionMapper extends BaseMapperPlus<LessonNodeQuizQuestion, LessonNodeQuizQuestion> {
    ...
    /** 查询题目总数（限定测验+租户）。 */
    @Select("SELECT COUNT(*) FROM node_quiz_questions WHERE quiz_id = #{quizId}::uuid AND tenant_id = #{tenantId}::uuid")
    long countQuestions(@Param("quizId") String quizId, @Param("tenantId") String tenantId);

    /** 题目列表（按 sort_order 升序，limit/offset 分页）。 */
    @Select("SELECT id, quiz_id, type, question, options::text AS options, answer, score, sort_order, tenant_id"
        + " FROM node_quiz_questions WHERE quiz_id = #{quizId}::uuid AND tenant_id = #{tenantId}::uuid"
    ...
    /** 查询单个题目（限定租户）。 */
    @Select("SELECT id, quiz_id, type, question, options::text AS options, answer, score, sort_order, tenant_id"
        + " FROM node_quiz_questions WHERE id = #{id}::uuid AND tenant_id = #{tenantId}::uuid")
    ...
    @Insert("INSERT INTO node_quiz_questions (id, tenant_id, quiz_id, type, question, options, answer, score, sort_order)"
        + " VALUES (gen_random_uuid(), #{tenantId}::uuid, #{quizId}::uuid, #{type}, #{question},"
        + " CAST(#{options} AS jsonb), #{answer}, #{score}, #{sortOrder}) RETURNING id")
    ...
    @Update("UPDATE node_quiz_questions SET type = #{type}, question = #{question},"
        + " options = CAST(#{options} AS jsonb), answer = #{answer}, score = #{score}, sort_order = #{sortOrder}"
    ...
    /** 删除题目（限定租户）。 */
    @Delete("DELETE FROM node_quiz_questions WHERE id = #{id}::uuid AND tenant_id = #{tenantId}::uuid")
    int deleteQuestion(@Param("id") String id, @Param("tenantId") String tenantId);

    /** 删除测验下的全部题目（删除测验时事务内级联清理，限定租户）。 */
    @Delete("DELETE FROM node_quiz_questions WHERE quiz_id = #{quizId}::uuid AND tenant_id = #{tenantId}::uuid")
