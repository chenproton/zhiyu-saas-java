...
public interface AllianceAchievementMapper extends BaseMapperPlus<AllianceAchievement, AllianceAchievement> {
    ...
    @Insert("INSERT INTO alliance_achievements (id, tenant_id, title, type, description, achievement_date,"
        ...
        + " CAST(#{attachments} AS jsonb), #{citationReason}, CAST(#{images} AS jsonb), CAST(#{ownerPersons} AS jsonb),"
        + " CAST(#{coBuilders} AS jsonb), CAST(#{enterpriseIds} AS jsonb), CAST(#{projectIds} AS jsonb),"
        + " CAST(#{relatedPositions} AS jsonb), CAST(#{relatedScenes} AS jsonb), CAST(#{relatedCourses} AS jsonb),"
        + " #{status}, #{viewCount}, CAST(#{secondaryColleges} AS jsonb), #{isPublic}, #{createdBy}, NOW(), NOW())")
    ...
    @Update("UPDATE alliance_achievements SET title = #{title}, type = #{type}, description = #{description},"
        + " achievement_date = #{achievementDate}, cover_image = #{coverImage}, attachments = CAST(#{attachments} AS jsonb),"
        + " citation_reason = #{citationReason}, images = CAST(#{images} AS jsonb), owner_persons = CAST(#{ownerPersons} AS jsonb),"
        + " co_builders = CAST(#{coBuilders} AS jsonb), enterprise_ids = CAST(#{enterpriseIds} AS jsonb),"
        + " project_ids = CAST(#{projectIds} AS jsonb), related_positions = CAST(#{relatedPositions} AS jsonb),"
        + " related_scenes = CAST(#{relatedScenes} AS jsonb), related_courses = CAST(#{relatedCourses} AS jsonb),"
        + " status = #{status}, secondary_colleges = CAST(#{secondaryColleges} AS jsonb), is_public = #{isPublic},"
