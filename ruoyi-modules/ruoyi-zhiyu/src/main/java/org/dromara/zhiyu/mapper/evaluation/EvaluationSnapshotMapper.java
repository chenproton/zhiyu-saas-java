...
public interface EvaluationSnapshotMapper extends BaseMapperPlus<EvaluationResourceSnapshot, EvaluationResourceSnapshot> {
    ...
    @Select("SELECT version FROM resource_snapshots"
        + " WHERE tenant_id = #{tenantId}::uuid AND resource_type = #{resourceType} AND resource_id = #{resourceId}::uuid"
    ...
    @Select("<script>"
        ...
        + " WHERE tenant_id = #{tenantId}::uuid AND id = #{resourceId}::uuid</script>")
