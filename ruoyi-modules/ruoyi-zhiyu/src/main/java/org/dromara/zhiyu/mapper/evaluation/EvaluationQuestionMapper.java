...
public interface EvaluationQuestionMapper extends BaseMapperPlus<EvaluationQuestion, EvaluationQuestion> {
    ...
    @Select("<script>SELECT id AS id, type, content, options, answer, analysis, score"
        + " FROM questions"
        + " WHERE JSON_CONTAINS(CAST(#{ids, typeHandler=org.dromara.zhiyu.core.mybatis.JsonStringArrayTypeHandler} AS JSON), JSON_QUOTE(id), '$')"
        + " AND tenant_id = #{tenantId}"
        + " ORDER BY array_position(#{ids, typeHandler=org.dromara.zhiyu.core.mybatis.JsonStringArrayTypeHandler}, id)</script>")
