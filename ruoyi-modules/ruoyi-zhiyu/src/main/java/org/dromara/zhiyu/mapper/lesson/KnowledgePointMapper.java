...
public interface KnowledgePointMapper extends BaseMapperPlus<KnowledgePoint, KnowledgePoint> {
    ...
    @Insert("INSERT INTO knowledge_points (id, tenant_id, name, code, description, linked, granular_lesson_ids, creator_id, source_type, source_id)"
        + " VALUES (#{id}, #{tenantId}::uuid, #{name}, #{code}, #{description}, #{linked},"
        + " CAST(#{granularLessonIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]),"
        + " #{creatorId}::uuid, #{sourceType}, #{sourceId}::uuid)")
    ...
    @Update("UPDATE knowledge_points SET name = #{name}, code = #{code}, description = #{description},"
        + " linked = #{linked}, granular_lesson_ids = CAST(#{granularLessonIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]),"
    ...
    @Update("UPDATE courses SET knowledge_point_ids = array_append(knowledge_point_ids, #{kpId}::uuid), updated_at = NOW()"
        + " WHERE tenant_id = #{tenantId} AND id = ANY(CAST(#{courseIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]))"
        + " AND NOT (#{kpId}::uuid = ANY(knowledge_point_ids))")
    int appendKpToCourses(@Param("kpId") String kpId, @Param("tenantId") String tenantId,
                          @Param("courseIds") List<String> courseIds);

    /** 同步颗粒课对知识点的双向引用：移除。 */
    @Update("UPDATE courses SET knowledge_point_ids = array_remove(knowledge_point_ids, #{kpId}::uuid), updated_at = NOW()"
        + " WHERE tenant_id = #{tenantId} AND id <> ALL(CAST(#{courseIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]))"
        + " AND #{kpId}::uuid = ANY(knowledge_point_ids)")
    int removeKpFromCourses(@Param("kpId") String kpId, @Param("tenantId") String tenantId,
                            @Param("courseIds") List<String> courseIds);

    /** 课程创建/更新时同步知识点颗粒课引用：追加课程 ID。 */
    @Update("UPDATE knowledge_points SET granular_lesson_ids = array_append(granular_lesson_ids, #{courseId}::uuid), updated_at = NOW()"
        + " WHERE tenant_id = #{tenantId} AND id = ANY(CAST(#{kpIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]))"
        + " AND NOT (#{courseId}::uuid = ANY(granular_lesson_ids))")
    ...
    /** 课程创建/更新时同步知识点颗粒课引用：移除课程 ID。 */
    @Update("UPDATE knowledge_points SET granular_lesson_ids = array_remove(granular_lesson_ids, #{courseId}::uuid), updated_at = NOW()"
        + " WHERE tenant_id = #{tenantId} AND id <> ALL(CAST(#{kpIds, typeHandler=org.dromara.zhiyu.core.mybatis.PgArrayTypeHandler} AS uuid[]))"
        + " AND #{courseId}::uuid = ANY(granular_lesson_ids)")
    ...
    @Select("<script>SELECT COUNT(*) FROM knowledge_points kp WHERE kp.tenant_id = #{tenantId}"
        ...
        + " AND NOT EXISTS (SELECT 1 FROM questions q WHERE kp.id = ANY(q.knowledge_point_ids))</script>")
