...
public interface PartnerScenarioMapper extends BaseMapperPlus<SceneScenario, SceneScenario> {
    ...
    @Select("<script>SELECT sc.id::text FROM scenarios sc"
        + " WHERE (sc.source_enterprise_id = #{enterpriseId}::uuid"
        + "   OR EXISTS (SELECT 1 FROM alliance_resource_grants g WHERE g.enterprise_id = #{enterpriseId}::uuid AND g.resource_type = 'scenario' AND sc.id = ANY(g.resource_ids)))"
        + " <if test=\"schoolTenantId != null and schoolTenantId != ''\"> AND sc.tenant_id = #{schoolTenantId}::uuid</if>"
        + " <if test=\"search != null and search != ''\"> AND sc.name ILIKE '%' || #{search} || '%'</if>"
    ...
    @Select("<script>SELECT COUNT(*) FROM scenarios sc"
        ...
        + "   OR EXISTS (SELECT 1 FROM alliance_resource_grants g WHERE g.enterprise_id = #{enterpriseId}::uuid AND g.resource_type = 'scenario' AND sc.id = ANY(g.resource_ids)))"
        + " <if test=\"schoolTenantId != null and schoolTenantId != ''\"> AND sc.tenant_id = #{schoolTenantId}::uuid</if>"
        + " <if test=\"search != null and search != ''\"> AND sc.name ILIKE '%' || #{search} || '%'</if></script>")
    ...
    @Select("SELECT status FROM scenarios WHERE id = #{id}::uuid")
    String selectStatus(@Param("id") String id);

    @Select("SELECT EXISTS(SELECT 1 FROM scenarios WHERE tenant_id = #{tenantId}::uuid AND code = #{code})")
    boolean existsCode(@Param("tenantId") String tenantId, @Param("code") String code);

    @Select("SELECT tenant_id::text FROM scenarios WHERE id = #{id}::uuid")
    String selectTenantId(@Param("id") String id);

    @Select("SELECT id::text FROM scenarios WHERE source_enterprise_id = #{enterpriseId}::uuid"
    ...
    @Select("SELECT EXISTS(SELECT 1 FROM scene_evaluation_results ser WHERE ser.scene_id = #{id}::uuid"
        + " OR ser.task_id IN (SELECT id FROM scenario_tasks WHERE scenario_id = #{id}::uuid))")
    boolean existsEvaluationResults(@Param("id") String id);

    @Update("UPDATE teaching_plan_entries SET scenario_id = NULL WHERE scenario_id = #{id}::uuid")
    int unbindTeachingPlanEntries(@Param("id") String id);

    @Update("UPDATE schedule_entries SET scenario_id = NULL WHERE scenario_id = #{id}::uuid")
    int unbindScheduleEntries(@Param("id") String id);

    @Delete("DELETE FROM scenarios WHERE id = #{id}::uuid")
